...
Consumes the :mod:`slm_training.dsl.analysis.arity.coding` layer (which is in turn
backed by the canonical CAP0-02 ``min_alphabet_for_capacity`` for its capacity
search) to emit the robust-coding arms that survive CAP0-03 scrutiny for a given
state-count target, and to drop the two arms CAP0-03 disproves. Torch-free.
"""
...
def smallest_feasible_alphabet(state_count: int, dimensions: int) -> int:
    """Smallest q such that q^dimensions >= state_count and Singleton admits it.
    ...
    # Conservatively require that Singleton with d=3 admits it (the toy robust bound).
