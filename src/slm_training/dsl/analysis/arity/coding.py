...
These are self-contained, Torch-free coding-theory helpers layered on top of the
canonical CAP0-02 arity package. They do not re-derive the arity counts; they
consume a target *state count* (the minimised-DFA class count a CAP0-02
:class:`~slm_training.dsl.analysis.arity.report.ExactArityReport` certifies) and
answer robust-coding feasibility questions about it.

The single point of contact with the canonical arity API is
:func:`smallest_injective_arity`, which delegates the ``K**d >= states`` capacity
search to :func:`slm_training.dsl.analysis.arity.report.min_alphabet_for_capacity`
so the exact integer computation has one source of truth (no float ``pow`` /
``ceil`` rounding). Everything else here is pure coding theory (Hamming balls,
sphere-packing / Singleton / Gilbert-Varshamov bounds, exhaustive minimum-distance
verification, and two locally verified small constructions).

Honesty scope: the two constructions are verified *exhaustively for their own
committed parameters only*; no arbitrary ``A_q(n, d)`` solver is claimed. See
...
from slm_training.dsl.analysis.arity.report import min_alphabet_for_capacity
...
def smallest_injective_arity(states: int, dimensions: int) -> int:
    ...
    Delegates the exact search to the canonical
    :func:`slm_training.dsl.analysis.arity.report.min_alphabet_for_capacity`
    (pure integer arithmetic — no float ``pow`` / ``ceil`` rounding), keeping a
    single source of truth for the ``K^d`` capacity computation shared with the
    CAP0-02 capacity row.
    """
    if states < 0 or dimensions <= 0:
        raise ValueError("states must be non-negative and dimensions positive")
    if states <= 1:
        return 1
    return min_alphabet_for_capacity(states, dimensions)
