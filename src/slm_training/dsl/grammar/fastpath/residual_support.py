...
circuit. When Γ is applied only as external leaf filters, results are labeled
``honest_overapprox`` (static CFG residual may be a superset of Γ-tight legality).
...
class ResidualSupportResult:
    ...
    admitted: bool
    authority: str  # "exact" | "honest_overapprox"
    ...
    def __post_init__(self) -> None:
        ...
        if self.authority not in {"exact", "honest_overapprox"}:
...
def joint_multi_hole_support(
    ...
    *,
    ...
    InteractiveParser prefix). When ``gamma_leaf_filters`` is True the static
    residual is an honest over-approximation of the fully Γ-tight domain —
    callers must not treat True as a certified Γ-exact ship claim without a
    separate leaf-filter pass.
    """
