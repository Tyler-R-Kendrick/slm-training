...
class TopologyAction(IntEnum):
    """Bounded topology edit actions (mirrors models.grammar_diffusion.TopologyAction)."""
...
class TopologyEdit:
    ...
    def from_value(cls, value: DomainValue) -> TopologyEdit:
        ...
        return cls(
            action=TopologyAction[payload["action"]],
...
def _selected_nodes(
    ...
    return [node for node in nodes if node.node_id in wanted][: config.topology_max_nodes]
...
def derive_topology_holes(
    ...
    *,
    ...
    """Map active topology nodes to finite semantic holes with complete edit tuples."""
