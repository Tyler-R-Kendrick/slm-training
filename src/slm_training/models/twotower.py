...
class TwoTowerModel(nn.Module):
        ...
        if pattern == "mixed" and stmt_p > 0.0:
            try:
                ...
                if is_dsl_native_tokenizer(self.tokenizer):
                    ...
                    # Derive stable statement spans from newline token ids so
                    # mixed masking is not silently identical to random masking.
    ...
    def _merge_ltr_suffix_mask(
        ...
        for i in range(bsz):
            has_bos = int(target_ids[i, 0]) == self.tokenizer.bos_id
            first_content = 1 if has_bos else 0
            cut = self._rng.randint(first_content, max(first_content, seq - 1))
            # Always train the first content decision. Lexer-native targets are
            # intentionally encoded without BOS, so position 0 is ``root``;
            # compositional targets retain the historical post-BOS position 1.
            if int(target_ids[i, first_content]) != self.tokenizer.pad_id:
                ltr_suffix[i, first_content] = True
                if not bool(predict_mask[i, first_content].item()):
                    predict_mask[i, first_content] = True
                    noisy[i, first_content] = self.tokenizer.mask_id
