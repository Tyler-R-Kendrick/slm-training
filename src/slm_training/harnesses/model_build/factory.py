...
def apply_runtime_overrides(model: Any, config: ModelBuildConfig) -> Any:
    ...
    for key in (
        ...
        "schema_identifier_decode_weight",
...
def _twotower_config_from_build(config: ModelBuildConfig) -> "TwoTowerConfig":
    ...
    return TwoTowerConfig(
        ...
        schema_enum_close_decode_weight=float(
            getattr(config, "schema_enum_close_decode_weight", 0.0) or 0.0
        ),
        ...
        schema_opaque_close_decode_weight=float(
            getattr(config, "schema_opaque_close_decode_weight", 0.0) or 0.0
        ),
        schema_role_slot_decode_weight=float(
            getattr(config, "schema_role_slot_decode_weight", 0.0) or 0.0
        ),
        schema_identifier_decode_weight=float(
            getattr(config, "schema_identifier_decode_weight", 0.0) or 0.0
        ),
        ...
        semantic_plan_margin_decode_weight=float(
            getattr(config, "semantic_plan_margin_decode_weight", 0.0) or 0.0
        ),
        semantic_plan_seed_decode_weight=float(
            getattr(config, "semantic_plan_seed_decode_weight", 0.0) or 0.0
        ),
        semantic_plan_inline_decode_weight=float(
            getattr(config, "semantic_plan_inline_decode_weight", 0.0) or 0.0
        ),
        semantic_plan_binding_decode_weight=float(
            getattr(config, "semantic_plan_binding_decode_weight", 0.0) or 0.0
        ),
        semantic_plan_root_decode_weight=float(
            getattr(config, "semantic_plan_root_decode_weight", 0.0) or 0.0
        ),
