...
def cli_lever_overrides(
    args: Any, *, argv: Sequence[str] | None = None
) -> dict[str, Any]:
    """Collect only explicitly supplied CLI lever overrides.

    ``argparse`` materializes defaults, so a Namespace alone cannot distinguish
    ``--solver-max-nodes 512`` from an omitted option. The raw argv preserves
    provider/ruleset precedence for omitted options.
    """
    ...
    if supplied("--compiler-decode-mode") and mode is not None:
    ...
    for key in ("solver_max_nodes", "solver_unknown_policy", "solver_certificate_mode"):
        if key in LEVER_BY_KEY and hasattr(args, key) and supplied(
            f"--{key.replace('_', '-')}"
        ):
