...
| --- | --- | --- | --- |
| Autotrain 8c0b60dd-c3 component-plan structural win, frozen-replay reproduction (session ctyadc) | 2 size-matched 1,755,764-param scratch checkpoints | `outputs/autoresearch/continuous-loop-20260803-continuous-openui-local-8c0b60dd-c3/runs/` (local, explicit no-sync) | `component-plan` beats control on `structural_similarity` by `+.05613`, byte-identical to every prior independent measurement of this hypothesis (PR #1369, #1376, #1378). Frozen replay of this session's own c2, closing out its git-mutation-interrupted measurement. Screening only; never reusable/promoted/synced/ship ([results](docs/design/continuous-openui-local-ctyadc-c3-results.md)) |
| Autotrain 8c0b60dd-c2 component-plan measurement incomplete, scheduled (session ctyadc) | 1 candidate-only 1,755,764-param scratch checkpoint; control not executed | `outputs/autoresearch/continuous-loop-20260803-continuous-openui-local-8c0b60dd-c2/runs/` (local, explicit no-sync) | Candidate structure `.3828` matches prior component-plan reproductions, but the matched control never ran: this session amended the campaign's locked `integration_commit` mid-cycle (operator error, not a harness bug). No attribution claim; `retry_measurement` queued for cycle 3. Never reusable/promoted/synced/ship ([results](docs/design/continuous-openui-local-ctyadc-c2-results.md)) |
| Autotrain 8c0b60dd-c1 bounds/control exact tie, scheduled reproduction (session ctyadc) | 2 size-matched 1,608,962-param scratch checkpoints (byte-identical to session j48f8u's c1) | `outputs/autoresearch/continuous-loop-20260803-continuous-openui-local-8c0b60dd-c1/runs/` (local, explicit no-sync) | Bounds vs control exact tie on every smoke quality metric (structure `.0575`, binder F1 `.6333`); only p50 latency differs (`3186.42` vs `3145.09` ms), not a metric win. Third reproduction of this null screening arm. Never reusable/promoted/synced/ship ([results](docs/design/continuous-openui-local-ctyadc-c1-results.md)) |
