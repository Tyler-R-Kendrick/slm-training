...
| --- | --- | --- | --- |
| Autotrain local-loop c2 component-plan measurement (incomplete) | 2 matched 1,755,764-param scratch checkpoints | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-local-8c0b60dd-c2/runs/` (local, explicit no-sync) | Control eval hit the harness wall-time cap before a scoreboard; no matched comparison exists. Inconclusive, not model evidence; `retry_measurement` queued to replay the identical frozen arms ([results](docs/design/autotrain-cycle-8c0b60dd-c2-component-plan-timeout.md)) |
| Autotrain local-loop c1 grammar_completion_bounds screen | 2 exactly matched 1,608,962-param scratch checkpoints | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-local-8c0b60dd-c1/runs/` (local, explicit no-sync) | First cycle of a fresh local loop (prior `202607-98199209` state not recoverable in this checkout). Exact smoke n=3 quality ties; bounds is 4.08% faster p50 with no quality change. Gates fail; neither checkpoint reusable, promoted, synced, or ship ([results](docs/design/autotrain-cycle-8c0b60dd-c1-grammar-bounds-screen.md)) |
