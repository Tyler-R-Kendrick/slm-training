...
| --- | --- | --- | --- |
| Autotrain (continuous-openui-local) c2 component-plan decode timeout | 2 size-matched 1,755,764-param CPU scratch checkpoints | `outputs/autoresearch/continuous-loop-20260804-continuous-openui-local-8c0b60dd-c2/runs/` (local, explicit no-sync) | Measurement incomplete: all 3 smoke documents decode-timed-out on both arms (0/3 completed); no model attribution. Investigated as a capacity edge (recipe compiler time ~ chunk decode budget), not a code defect. **Replay queued; no promote/sync/ship** ([results](docs/design/autotrain-cycle-20260804-c2-component-plan-decode-timeout.md)) |
| Autotrain (continuous-openui-local) c1 bounds screen | 2 size-matched 1,608,962-param CPU scratch checkpoints | `outputs/autoresearch/continuous-loop-20260804-continuous-openui-local-8c0b60dd-c1/runs/` (local, explicit no-sync) | Quality null: structure `.0575` tie, meaningful `0`, binder F1 `.6333`, parse `1.0`; candidate p50 `4614.58` vs `4726.59` ms. Smoke `n=3`, full suites missing. **Rejected (SDLC Phase A NON_POSITIVE); no promote/sync/ship** ([results](docs/design/autotrain-cycle-20260804-c1-bounds-screen.md)) |
