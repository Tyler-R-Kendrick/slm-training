...
| --- | --- | --- | --- |
| Autotrain `continuous-openui-local` c2 component-inventory screen (session `dsz5wf`, 2026-08-05) | 2 size-matched CPU scratch checkpoints (trained, unevaluated) | `outputs/autoresearch/continuous-loop-20260805-continuous-openui-local-8c0b60dd-c2/runs/` (local, explicit no-sync) | First test of component-inventory this session via the new `--skip-slugs` rotation override; both arms trained but AgentV hit an internal decode timeout before either scoreboard completed (control `missing_scoreboard`, candidate 3/3 `decode_timeout_count`). **Harness-incomplete, not a model result; repair_harness queued; never promote/sync/ship** ([results](docs/design/continuous-openui-20260805-dsz5wf-c2-results.md)) |
| Autotrain `continuous-openui-local` c1 bounds screen (session `dsz5wf`, 2026-08-05) | 2 size-matched 1,608,962-param CPU scratch checkpoints | `outputs/autoresearch/continuous-loop-20260805-continuous-openui-local-8c0b60dd-c1/runs/` (local, explicit no-sync) | Independent same-day reproduction of PR #1450's c1 (same recipe/seed): 4th exact `structural_similarity` tie (`.0575`), 3rd+ latency sign-reversal (`bounds` 33.5% slower this run vs faster in #1450). **Rejected/exhausted; never promote/sync/ship** ([results](docs/design/continuous-openui-20260805-dsz5wf-c1-results.md)) |
