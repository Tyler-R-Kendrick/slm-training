...
| --- | --- | --- | --- |
| Autotrain mpjoo9-c2 component-plan wall-timeout | 2 size-matched 1,755,764-param scratch checkpoints | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-mpjoo9-2b5d9d52-c2/runs/` (local, explicit no-sync) | Both 22-step CPU arms trained, but the control's evaluation was skipped (per-arm wall budget exhausted) so there is no honest A/B comparison; candidate alone scores parse 1.0, structure .0964, and fails `--ship-gates` with AgentV SDK unavailable. Rejected; neither checkpoint reusable, promoted, synced, or ship ([results](docs/design/autotrain-mpjoo9-c2-component-plan-timeout.md)) |
| Autotrain mpjoo9-c1 bounds screen | 2 exactly matched 1,608,962-param scratch checkpoints | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-mpjoo9-2b5d9d52-c1/runs/` (local, explicit no-sync) | Both 21-step CPU arms are exact smoke n=3 quality ties: parse 1.0, binder F1 .6333, meaning 0, structure .0575. Candidate p50 is 2.33% slower (2,275.05→2,328.11 ms). `--ship-gates` publish fails with AgentV SDK unavailable (no Node.js/npm in this sandbox); base suite scoring completed. Rejected; neither checkpoint reusable, promoted, synced, or ship ([results](docs/design/autotrain-mpjoo9-c1-bounds-screen.md)) |
