...
| --- | --- | --- | --- |
| Autotrain `continuous-openui-local` c1 bounds screen | 2 size-matched 1,608,962-param scratch checkpoints | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-local-8c0b60dd-c1/runs/` (local, explicit no-sync) | `fixture_or_scratch`: CPU, `wf_smoke_v2`, 21 steps, seed 100001, batch 2. Exact smoke n=3 quality tie (parse 1.0, meaning 0, structure .0575); bounds p50 1553.54 vs control 1594.76 ms. AgentV publish crashed on a missing `@agentv/core` install, so both arms are `measurement_incomplete`, not a scored comparison; `npm ci` fixed the install and frozen replay is queued next. Neither checkpoint is reusable, promoted, synced, or ship ([results](docs/design/continuous-openui-20260802-c1-results.md)) |
