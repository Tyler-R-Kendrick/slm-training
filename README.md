...
| --- | --- | --- | --- |
| Autotrain continuous-loop `0gz1bq` c5 component-plan decode timeout | 2 size-matched 1,755,760-param CPU scratch checkpoints | `outputs/autoresearch/continuous-loop-20260805-continuous-openui-local-8c0b60dd-c5/runs/` (local, explicit no-sync) | control completed; component-plan timed out 3/3 documents at the 36s decode ceiling — a capacity signal, not a code defect. **No comparison; never reusable/promoted/synced/ship** ([results](docs/design/continuous-openui-local-0gz1bq-c5-results.md)) |
| Autotrain continuous-loop `0gz1bq` c4 component-edge exact tie, slower | 2 size-matched 1,766,990-param CPU scratch checkpoints | `outputs/autoresearch/continuous-loop-20260805-continuous-openui-local-8c0b60dd-c4/runs/` (local, explicit no-sync) | Byte-identical smoke quality both arms; component-edge is +15.9% p50 slower with zero quality gain. **Non-positive; never reusable/promoted/synced/ship** ([results](docs/design/continuous-openui-local-0gz1bq-c4-results.md)) |
| Autotrain continuous-loop `0gz1bq` c3 component-plan regression (5th measurement) | 2 size-matched 1,755,760-param CPU scratch checkpoints | `outputs/autoresearch/continuous-loop-20260805-continuous-openui-local-8c0b60dd-c3/runs/` (local, explicit no-sync) | Fresh-seed test of `component-plan` regresses `structural_similarity` -0.05833 and `binder_reference_f1` vs control; three sessions now disagree on this hypothesis's sign (win/tie/regress). **Non-positive; treat as exhausted; never reusable/promoted/synced/ship** ([results](docs/design/continuous-openui-local-0gz1bq-c3-results.md)) |
| Autotrain continuous-loop `0gz1bq` c2 grammar-completion-bounds replay | 2 size-matched 1,608,962-param CPU scratch checkpoints | `outputs/autoresearch/continuous-loop-20260805-continuous-openui-local-8c0b60dd-c2/runs/` (local, explicit no-sync) | Frozen replay of the `grammar-completion-bounds` hypothesis after a fresh-checkout torch-install self-heal (c1); exact tie on every smoke quality metric, fourth independent session to reproduce this null. **Non-positive; never reusable/promoted/synced/ship** ([results](docs/design/continuous-openui-local-0gz1bq-c2-results.md)) |
