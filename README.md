...
| --- | --- | --- | --- |
| Autotrain 8c0b60dd-c2 component-plan host-speed eval timeout (session n8vwtq) | 2 size-matched scratch checkpoints (byte-identical to `j48f8u` c2) | `outputs/autoresearch/continuous-loop-20260803-continuous-openui-local-8c0b60dd-c2/runs/` (local, explicit no-sync) | Training completed; ship-gate eval timed out mid-decode (`compiler_decode_mode=tree` ~35.7s/document on this host). Host-speed soft timeout, not a model/harness defect; hypothesis already positively reproduced 3 times elsewhere (#1369, #1376, #1378). Never reusable/promoted/synced/ship ([results](docs/design/continuous-openui-local-n8vwtq-c2-results.md)) |
| Autotrain 8c0b60dd-c1 bounds/control exact tie (session n8vwtq) | 2 size-matched scratch checkpoints | `outputs/autoresearch/continuous-loop-20260803-continuous-openui-local-8c0b60dd-c1/runs/` (local, explicit no-sync) | Byte-identical training loss and exact smoke-metric tie between arms; only p50 latency differs (`1685.14` vs `1786.34` ms), which alone is not a metric win. Never reusable/promoted/synced/ship ([results](docs/design/continuous-openui-local-n8vwtq-c1-results.md)) |
