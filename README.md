...
| --- | --- | --- | --- |
| Autotrain scheduled-loop `continuous-openui-local` c4 (session `ttvqzi`) component-edge tie | 2 size-matched 1,766,990-param CPU scratch checkpoints | `outputs/autoresearch/continuous-loop-20260805-continuous-openui-local-8c0b60dd-c4/runs/` (local, explicit no-sync) | Exact tie on every smoke metric; candidate +11.2% p50 latency with no quality signal. **Rejected; no attribution/promote/sync/ship** ([results](docs/design/continuous-openui-local-ttvqzi-c4-results.md)) |
| Autotrain scheduled-loop `continuous-openui-local` c3 (session `ttvqzi`) component-plan regression | 2 size-matched 1,755,764-param CPU scratch checkpoints | `outputs/autoresearch/continuous-loop-20260805-continuous-openui-local-8c0b60dd-c3/runs/` (local, explicit no-sync) | `component-plan` regresses structure `.2308→.1725`, MPR `.333→0`, binder `.7333→.6333` at seed 100001 — opposite of an earlier session's win at seed 100002. **Seed-sensitivity evidence; no attribution/promote/sync/ship** ([results](docs/design/continuous-openui-local-ttvqzi-c3-results.md)) |
| Autotrain scheduled-loop `continuous-openui-local` c2 (session `ttvqzi`, retry_measurement) bounds rejected | 2 size-matched 1,608,962-param CPU scratch checkpoints | `outputs/autoresearch/continuous-loop-20260805-continuous-openui-local-8c0b60dd-c2/runs/` (local, explicit no-sync) | Frozen replay of c1's arm pair completed cleanly this time; exact tie on every smoke metric (structure `.0575`, MPR `0`). **Bounds hypothesis rejected (2nd clean measurement); no attribution/promote/sync/ship** ([results](docs/design/continuous-openui-local-ttvqzi-c2-results.md)) |
| Autotrain scheduled-loop `continuous-openui-local` c1 (session `ttvqzi`) measurement incomplete | 2 size-matched 1,608,962-param CPU scratch checkpoints | `outputs/autoresearch/continuous-loop-20260805-continuous-openui-local-8c0b60dd-c1/runs/` (local, explicit no-sync) | Control eval finished 3/3 documents but was killed by the `MAX_RUN_MINUTES=3` wall cap before a scoreboard was written; bounds completed (`exit=0`) but fails every quality/evidence-volume gate at fixture `n=3`. **Measurement incomplete; retry queued; no attribution/promote/sync/ship** ([results](docs/design/continuous-openui-local-ttvqzi-c1-results.md)) |
