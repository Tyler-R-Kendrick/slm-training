...
| --- | --- | --- | --- |
| Autotrain continuous-openui-local c1 bounds screen | 2 size-matched 1,608,960-param CPU scratch checkpoints | `outputs/autoresearch/continuous-loop-20260804-continuous-openui-local-8c0b60dd-c1/runs/` (local, explicit no-sync) | Quality null: structure `.0575` tie, meaningful `0`, binder F1 `.6333` tie, parse `1.0`; candidate p50 `3854.54` vs control `3299.53` ms. Smoke `n=3`, 11 honest ship-gate failures. **Rejected fixture null; no promote/sync/ship** ([results](docs/design/autotrain-continuous-openui-local-c1-20260804-screening.md)) |
