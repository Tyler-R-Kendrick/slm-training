...
| --- | --- | --- | --- |
| Autotrain continuous-openui-local c2 (session sk4t9p) component-plan win, 5th reproduction | 2 size-matched scratch checkpoints (1,755,764 params each) | `outputs/autoresearch/continuous-loop-20260803-continuous-openui-local-8c0b60dd-c2/runs/` (local, explicit no-sync) | `component-plan` beats control on structural_similarity +.05613 (.32667→.38280), seed 100002 — byte-identical to four prior sessions (PR #1369, #1376, #1378). Fixture screening only; ship gates fail (n=3) ([results](docs/design/continuous-openui-local-sk4t9p-c2-results.md)) |
| Autotrain continuous-openui-local c1 (session sk4t9p) knob-rotation null | 2 size-matched scratch checkpoints (1,608,962 params each) | `outputs/autoresearch/continuous-loop-20260803-continuous-openui-local-8c0b60dd-c1/runs/` (local, explicit no-sync) | `grammar_completion_bounds` arm ties control exactly on every smoke quality metric; latency-only delta is not a win. Rejected null ([results](docs/design/continuous-openui-local-sk4t9p-c1-results.md)) |
