...
| --- | --- | --- | --- |
| Autotrain c1726 held_out wall timeout | 2 matched 1,608,962-param CPU scratch checkpoints (21 vs 42 steps) | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-local-8c0b60dd-c4/runs/` (local, explicit no-sync) | Promotion-role cycle; `held_out` timed out at the 3-minute wall cap on both arms so no promotion metric exists. `steps` (42 steps) completed smoke only with the best smoke numbers in this loop yet (meaningful 0.3333, binder F1 0.8222, structure 0.51) but that is not the declared primary metric. Incomplete, not reusable/promoted/ship; frozen replay required ([results](docs/design/autotrain-cycle-1726-held-out-wall-timeout.md)) |
| Autotrain c1725 rotated lever | 2 matched 1,608,962-param CPU scratch checkpoints, distinct config hashes | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-local-8c0b60dd-c3/runs/` (local, explicit no-sync) | First real lever test in the `continuous-openui-local` loop (c1723/c1724 replayed a config-identical pair). Both arms smoke n=3 completed (parse 1.0, binder F1 0.7222, meaningful 0.0, structure 0.1725) and honestly failed gates; the rotated lever's smoke-scale metrics matched control — underpowered, not proof of an inert lever. Not reusable/promoted/ship ([results](docs/design/autotrain-cycle-1725-rotated-lever-null-delta.md)) |
