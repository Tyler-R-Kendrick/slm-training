...
| --- | --- | --- | --- |
| Autotrain c1737 component-plan screen (measurement incomplete) | 2 scratch checkpoints (control eval incomplete) | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-202607-98199209-c2/runs/` (local, explicit no-sync) | Control smoke decode wall-timed out before scoring; treatment (`component_plan_loss_weight=1.0`) completed alone and honestly fails every quality gate on fixture n=3 (binder F1 0, meaningful 0, structure .0964). No valid comparison this cycle; neither checkpoint reusable, promoted, synced, or ship ([results](docs/design/autotrain-cycle-1737-component-plan-timeout-screen.md)) |
| Autotrain c1736 grammar-completion-bounds screen | 2 exactly matched scratch checkpoints | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-202607-98199209-c1/runs/` (local, explicit no-sync) | Both 21-step CPU arms are exact smoke n=3 quality ties; treatment p50 is 1.46% faster (2,429.84→2,394.46 ms), but `meaningful_program_rate=0` is below the 1/3 floor required to credit a latency-only win. Rejected; neither checkpoint reusable, promoted, synced, or ship ([results](docs/design/autotrain-cycle-1736-grammar-completion-bounds-screen.md)) |
