...
| --- | --- | --- | --- |
| Autotrain scheduled-loop `continuous-openui-scheduled` c7 component-edge null | 2 size-matched 1,766,990-param CPU scratch checkpoints | `outputs/autoresearch/continuous-loop-20260805-continuous-openui-schedu-1e62ecf9-c7/runs/` (local, explicit no-sync) | Exact tie: structure `.0575` both arms — second consecutive `structural_aux_head_profile`-family exact tie. Smoke `n=3`, full suites missing. **Rejected null; no promote/sync/ship** ([results](docs/design/continuous-openui-scheduled-c7-results.md)) |
| Autotrain scheduled-loop `continuous-openui-scheduled` c6 component-plan null | 2 size-matched 1,755,760-param CPU scratch checkpoints | `outputs/autoresearch/continuous-loop-20260805-continuous-openui-schedu-1e62ecf9-c6/runs/` (local, explicit no-sync) | Exact tie: structure `.0964` both arms, meaningful `0`, binder F1 `.8222`. Smoke `n=3`, full suites missing. **Rejected null; no promote/sync/ship** ([results](docs/design/continuous-openui-scheduled-c6-results.md)) |
| Autotrain scheduled-loop `continuous-openui-scheduled` c5 frozen replay non-regression reject | 2 size-matched 1,608,960-param CPU scratch checkpoints | `outputs/autoresearch/continuous-loop-20260805-continuous-openui-schedu-1e62ecf9-c5/runs/` (local, explicit no-sync) | Frozen replay of c4: candidate wins structure `.4167→.51` and latency (-58%) but regresses binder F1 `.9524→.8222` — non-positive under the quality-aware tradeoff classifier. Smoke `n=3`, full suites missing. **No promote/sync/ship** ([results](docs/design/continuous-openui-scheduled-c5-results.md)) |
| Autotrain scheduled-loop `continuous-openui-scheduled` c4 `generate_batch_size` harness unblock | 1 completed control checkpoint (1,608,960 params); candidate `steps` arm never ran | `outputs/autoresearch/continuous-loop-20260805-continuous-openui-schedu-1e62ecf9-c4/runs/` (local, explicit no-sync) | First cycle in this loop's history to run end to end after fixing a `generate_batch_size` schema-drift harness bug (`5e36624a`) that had blocked every prior cycle with `CYCLE_ERROR`. **Proven executable unblock; model comparison inconclusive; no promote/sync/ship** ([results](docs/design/continuous-openui-scheduled-c4-results.md)) |
