...
| --- | --- | --- | --- |
| Autotrain `c3d4f8` c7 component-plan re-screen (session close) | 2 size-matched 1,755,764-param CPU scratch checkpoints | `outputs/autoresearch/continuous-loop-20260805-continuous-openui-c3d4f8-986c6dc3-c7/runs/` (local, explicit no-sync) | Null delta: structure `.0575` both arms, parse `1.0`, meaningful `0`; candidate p50 `4,733.17` vs `4,695.94` ms. Smoke `n=3`, full suites missing. **Rejected fixture null; never promote/sync/ship** ([results](docs/design/continuous-openui-c3d4f8-c7-results.md)) |
| Autotrain `c3d4f8` c6 component-edge screen | 2 size-matched 1,766,990-param CPU scratch checkpoints | `outputs/autoresearch/continuous-loop-20260805-continuous-openui-c3d4f8-986c6dc3-c6/runs/` (local, explicit no-sync) | Null delta: structure `.0964` both arms, parse `1.0`, meaningful `0`; candidate p50 `5,007.87` vs `4,343.63` ms. Smoke `n=3`, full suites missing. **Rejected fixture null; never promote/sync/ship** ([results](docs/design/continuous-openui-c3d4f8-c6-results.md)) |
| Autotrain `c3d4f8` c4 component-plan measurement incomplete | 1 control 1,755,764-param CPU scratch checkpoint (candidate never started) | `outputs/autoresearch/continuous-loop-20260805-continuous-openui-c3d4f8-986c6dc3-c4/runs/` (local, explicit no-sync) | Control-only: structure `.41667`, meaningful `.33333`, p50 `32,592.7` ms; candidate exceeded the 3-min wall cap before starting. **No model attribution; `retry_measurement` queued; never promote/sync/ship** ([results](docs/design/continuous-openui-c3d4f8-c4-results.md)) |
| Autotrain `c3d4f8` c3 both/control screen (harness repair replay) | 2 size-matched 1,608,962-param CPU scratch checkpoints | `outputs/autoresearch/continuous-loop-20260805-continuous-openui-c3d4f8-986c6dc3-c3/runs/` (local, explicit no-sync) | First successful cycle after repairing an `autoresearch`-family `generate_batch_size` schema block (commit `5381238`) that hard-failed cycles 1-2 before any experiment ran. Null delta: structure `.23083` both arms, parse `1.0`, meaningful `.33333`; candidate p50 `14,385.19` vs `13,815.51` ms. Smoke `n=3`, full suites missing. **Rejected fixture null; never promote/sync/ship** ([results](docs/design/continuous-openui-c3d4f8-c3-results.md)) |
