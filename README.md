...
| --- | --- | --- | --- |
| Autotrain c5 (continuous-openui-scheduled) decode-timeout blocked (3x) | Frozen replay of c2's 2 size-matched 1,608,962-param checkpoints | `outputs/autoresearch/continuous-loop-20260804-continuous-openui-schedu-1e62ecf9-c5/runs/` (local, explicit no-sync) | Third consecutive identical decode timeout (compiler_ms_mean ~21.3-23.3s vs fitted ~8.0s); loop-law repeated-hard-blocker threshold met. Formally blocked on this sandbox, not a code defect; no further retry this iteration ([results](docs/design/autotrain-cycle-continuous-openui-scheduled-c5-decode-timeout-blocked.md)) |
| Autotrain c4 (continuous-openui-scheduled) decode-timeout confirmed reproducible | Frozen replay of c2's 2 size-matched 1,608,962-param checkpoints | `outputs/autoresearch/continuous-loop-20260804-continuous-openui-schedu-1e62ecf9-c4/runs/` (local, explicit no-sync) | Warm-process replay reproduces c3's decode timeout within 1% (compiler_ms_mean ~23.1-23.2s both cycles) on an uncontended host; corrects c3's cold-sandbox hypothesis. Genuine sandbox-class decode-speed limit, not a code defect; no policy change, no further retry on this hardware ([results](docs/design/autotrain-cycle-continuous-openui-scheduled-c4-decode-timeout-confirmed.md)) |
| Autotrain c3 (continuous-openui-scheduled) decode-timeout diagnostic | Frozen replay of c2's 2 size-matched 1,608,962-param checkpoints | `outputs/autoresearch/continuous-loop-20260804-continuous-openui-schedu-1e62ecf9-c3/runs/` (local, explicit no-sync) | AgentV SDK self-heal worked, but every smoke record (3/3) timed out decode on both arms (compiler_ms_mean ≈23.2s vs fitted ≈8.0s budget). Diagnosed as a cold-sandbox anomaly, not a policy defect (historical smoke decode ~0.9-4.0s); no code/behavior change. Not scoreable; queued for retry with a warm process ([results](docs/design/autotrain-cycle-continuous-openui-scheduled-c3-decode-timeout-diagnostic.md)) |
| Autotrain c2 (continuous-openui-scheduled) AgentV infra recurrence, self-healed | 2 size-matched 1,608,962-param scratch checkpoints | `outputs/autoresearch/continuous-loop-20260804-continuous-openui-schedu-1e62ecf9-c2/runs/` (local, explicit no-sync) | Both arms trained (loss `14.390223503112793` each) but `--ship-gates` eval crashed on a missing AgentV SDK before any scoreboard; recurrence of the earlier c2 infra failure in a fresh checkout that skipped `scripts/setup_dev_env.sh`. Self-healed: `run_cycle` now bootstraps the SDK via `npm ci` itself. Not scoreable, not a model result; never reusable/promoted/synced/ship ([results](docs/design/autotrain-cycle-continuous-openui-scheduled-c2-agentv-missing-recurrence.md)) |
