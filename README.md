...
| --- | --- | --- | --- |
| Autotrain continuous-openui-local c6 component-inventory exact tie | 2 size-matched 1,682,360-param scratch checkpoints | `outputs/autoresearch/continuous-loop-20260803-continuous-openui-local-8c0b60dd-c6/runs/` (local, explicit no-sync) | Every quality metric ties exactly (structure `.0964`, binder F1 `.82222` both); only p50 differs `1119.80` vs `1164.64` ms. Fifth screening arm at this recipe to tie; rejected, never reusable/promoted/synced/ship ([results](docs/design/continuous-openui-20260803-c6-results.md)) |
| Autotrain continuous-openui-local c5 component-edge exact tie | 2 size-matched 1,766,992-param scratch checkpoints | `outputs/autoresearch/continuous-loop-20260803-continuous-openui-local-8c0b60dd-c5/runs/` (local, explicit no-sync) | Every quality metric ties exactly (structure `.04307`, binder F1 `.82222` both); only p50 differs `4539.33` vs `4852.35` ms. Rejected, never reusable/promoted/synced/ship ([results](docs/design/continuous-openui-20260803-c5-results.md)) |
| Autotrain continuous-openui-local c4 batch1 latency-only delta | 2 size-matched 1,608,960-param scratch checkpoints | `outputs/autoresearch/continuous-loop-20260803-continuous-openui-local-8c0b60dd-c4/runs/` (local, explicit no-sync) | Every quality metric ties exactly (structure `.41667`, binder F1 `.95238` both); p50 improves 16.6% but the climb-policy rule blocks an efficiency ratio from overriding a tied quality primary. Rejected; never reusable/promoted/synced/ship ([results](docs/design/continuous-openui-20260803-c4-results.md)) |
| Autotrain continuous-openui-local c3 component-plan confirmation rejected | 2 size-matched 1,755,764-param scratch checkpoints | `outputs/autoresearch/continuous-loop-20260803-continuous-openui-local-8c0b60dd-c3/runs/` (local, explicit no-sync) | Fresh-seed confirmation of the c2 component-plan champion: primary ties exactly and binder F1/fidelity regress (`.48889→.26667`, `.38889→.22222`). `CHAMPION_STATUS=rejected` — the c2 structural win does not replicate; never reusable/promoted/synced/ship ([results](docs/design/continuous-openui-20260803-c3-results.md)) |
