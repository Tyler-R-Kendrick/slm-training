...
| --- | --- | --- | --- |
| Autotrain `continuous-openui-local` c4 component-edge screen (2026-08-05) | 2 size-matched 1,766,987-param CPU scratch checkpoints | `outputs/autoresearch/continuous-loop-20260805-continuous-openui-local-8c0b60dd-c4/runs/` (local, explicit no-sync) | Every guarded quality metric exact tie; candidate p50 7.6% faster is efficiency-only (structural_similarity delta `0.0`). **Rejected as non-positive; never promote/sync/ship** ([results](docs/design/continuous-openui-20260805-8c0b60dd-c4-results.md)) |
| Autotrain `continuous-openui-local` c2 component-plan screen (2026-08-05) | 2 size-matched 1,755,764-param CPU scratch checkpoints | `outputs/autoresearch/continuous-loop-20260805-continuous-openui-local-8c0b60dd-c2/runs/` (local, explicit no-sync) | Control hit 3/3 decode timeouts (no scoreboard); component-plan candidate completed (`structural_similarity=.3828`) but no matched-control delta is attributable. **Inconclusive; retry_measurement queued; never promote/sync/ship** ([results](docs/design/continuous-openui-20260805-8c0b60dd-c2-results.md)) |
| Autotrain `continuous-openui-local` c1 bounds screen (2026-08-05) | 2 size-matched 1,608,962-param CPU scratch checkpoints | `outputs/autoresearch/continuous-loop-20260805-continuous-openui-local-8c0b60dd-c1/runs/` (local, explicit no-sync) | First cycle after #1444's `generate_batch_size` knob unblock; `grammar_completion_bounds` ties control on `structural_similarity` for the 3rd time (exhausted hypothesis), latency delta is a 2nd sign-reversal (sandbox noise, not attributable). **Rejected; never promote/sync/ship** ([results](docs/design/continuous-openui-20260805-8c0b60dd-c1-results.md)) |
