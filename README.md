...
| --- | --- | --- | --- |
| Autotrain c4 component-edge decode-timeout blocked | 2 size-matched 1,766,990-param CPU scratch checkpoints | `outputs/autoresearch/continuous-loop-20260804-continuous-openui-local-8c0b60dd-c4/runs/` (local, explicit no-sync) | Both arms trained but every smoke document hit the 8.0s decode-timeout wall (`compiler_ms_mean` ~23s, ~3x budget) on this CPU-only sandbox. No scoreboard; queued for CPU-only decode-timeout/arm-size harness repair. **Incomplete measurement; no promote/sync/ship** ([results](docs/design/continuous-openui-local-20260804-c4-results.md)) |
| Autotrain c3 component-plan regression | 2 size-matched 1,755,764-param CPU scratch checkpoints | `outputs/autoresearch/continuous-loop-20260804-continuous-openui-local-8c0b60dd-c3/runs/` (local, explicit no-sync) | Structure `.2308→.1725`, binder F1 `.7333→.6333`, meaningful `.333→0`; candidate p50 `12,685.96→11,866.74` ms. Smoke `n=3`, full suites missing. **Rejected; no promote/sync/ship** ([results](docs/design/continuous-openui-local-20260804-c3-results.md)) |
| Autotrain c2 bounds frozen replay | 2 size-matched 1,608,962-param CPU scratch checkpoints | `outputs/autoresearch/continuous-loop-20260804-continuous-openui-local-8c0b60dd-c2/runs/` (local, explicit no-sync) | Frozen replay of c1 after `npm ci` harness repair; identical structure `.0575` both arms, meaningful `0`, binder F1 `.6333`, parse `1.0`. Smoke `n=3`, full suites missing. **Null delta, rejected; no promote/sync/ship** ([results](docs/design/continuous-openui-local-20260804-c2-results.md)) |
| Autotrain c1 bounds harness failure | 2 size-matched 1,608,962-param CPU scratch checkpoints | `outputs/autoresearch/continuous-loop-20260804-continuous-openui-local-8c0b60dd-c1/runs/` (local, explicit no-sync) | Both arms trained but evaluation failed (`AgentV SDK unavailable`, fresh-container `npm ci` gap; repaired and replayed as c2). No scoreboard this cycle. **Incomplete measurement; no promote/sync/ship** ([results](docs/design/continuous-openui-local-20260804-c1-results.md)) |
