...
| --- | --- | --- | --- |
| Autotrain bqw0tb c5 timeout pair | 2 matched CPU scratch TwoTower checkpoints (`c5-control`, `c5-batch1`) | `outputs/autoresearch/continuous-loop-20260801-continuous-openui-bqw0tb-f852ac38-c5/runs/*/checkpoints/last.pt` (local, explicit no-sync) | Control completed (fixture gate fail, expected); `batch_size=1` treatment arm timed out mid-decode under the wall cap — soft failure, never evidence, not a harness defect. Frozen replay required ([results](docs/design/continuous-openui-bqw0tb-c5-results.md)); not promoted, reused, or shipped |
| Autotrain bqw0tb c4 screening pair | 2 matched CPU scratch TwoTower checkpoints (`c4-control`, `c4-both`) | `outputs/autoresearch/continuous-loop-20260801-continuous-openui-bqw0tb-f852ac38-c4/runs/*/checkpoints/last.pt` (local, explicit no-sync) | Frozen replay of c3 after the AgentV SDK fix. Smoke `n=3`: `binder_reference_f1` 0.7222 identical on both arms (0.0 delta); combined `grammar_completion_bounds`+`compact_active_canvas` costs +661.72ms p50 latency with no metric win; ship gates fail on fixture n—not promoted, reused, or shipped ([results](docs/design/continuous-openui-bqw0tb-c4-results.md)) |
| Autotrain bqw0tb c3 infra-fail pair | 2 matched CPU scratch TwoTower checkpoints (`c3-control`, `c3-both`) | `outputs/autoresearch/continuous-loop-20260801-continuous-openui-bqw0tb-f852ac38-c3/runs/*/checkpoints/last.pt` (local, explicit no-sync) | Both arms trained to completion (`binder_reference_f1` 0.7222 identical), but eval publication crashed on a missing/misconfigured AgentV SDK (no `node_modules/@agentv/core` + sandbox `NODE_OPTIONS` rejected by Node). Infrastructure-only; frozen replay required after `npm ci` + `NODE_OPTIONS` sanitization ([results](docs/design/continuous-openui-bqw0tb-c3-results.md)); never reuse, promote, or ship |
| Autotrain bqw0tb c2 screening pair | 2 matched CPU scratch TwoTower checkpoints (`c2-control`, `c2-bounds`) | `outputs/autoresearch/continuous-loop-20260801-continuous-openui-bqw0tb-f852ac38-c2/runs/*/checkpoints/last.pt` (local, explicit no-sync) | 21-step `wf_smoke_v2` replay after the c1 torch-missing infra fail. Smoke `n=3`: `binder_reference_f1` 0.6333 identical on both arms (0.0 delta), `meaningful_program_rate` 0.0, honest ship gates fail. `grammar_completion_bounds` gave no primary-metric win and higher latency—not promoted, reused, or shipped ([results](docs/design/continuous-openui-bqw0tb-c2-results.md)) |
