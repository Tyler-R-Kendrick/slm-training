...
| --- | --- | --- | --- |
| Autotrain c1865 c2 decode-timeout harness failure, repaired | 2 size-matched 1,755,764-param CPU scratch checkpoints (measurement incomplete) | `outputs/autoresearch/continuous-loop-20260804-continuous-openui-schedu-6699f447-c2/runs/` (local, explicit no-sync) | No scoreboard on either arm: `decode_timeout_count=3/3`, every record clipped at exactly `8000.33` ms (per-chunk timeout grant, zero margin). Repaired same cycle (`screening_decode_timeout_seconds` `8→10`); frozen arm queued for replay. **Not a model result; no promote/sync/ship** ([results](docs/design/autotrain-cycle-1865-c2-decode-timeout-harness-failure-repaired.md)) |
| Autotrain c1864 bounds-lever latency win, quality null | 2 size-matched 1,608,962-param CPU scratch checkpoints | `outputs/autoresearch/continuous-loop-20260804-continuous-openui-schedu-6699f447-c1/runs/` (local, explicit no-sync) | `grammar_completion_bounds=true` cuts p50 `10,815.8→5,639.31` ms (≈48%) with parse held `1.0`, but MPR stays `0` and structure/binder/fidelity are unchanged. Smoke `n=3`. **Latency-only; does not clear the quality-tradeoff gate; no promote/sync/ship** ([results](docs/design/autotrain-cycle-1864-bounds-lever-latency-win-quality-null.md)) |
