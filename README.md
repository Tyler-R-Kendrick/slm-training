...
| --- | --- | --- | --- |
| Autotrain c3 (campaign `8c0b60dd-c1`) bounds null delta | 2 size-matched 1,608,962-param scratch checkpoints | `outputs/autoresearch/continuous-loop-20260803-continuous-openui-local-8c0b60dd-c1/runs/` (local, explicit no-sync) | Bounds vs control exact tie on smoke structure `.0575` both, MPR/recall 0 both, binder F1 `.6333` both; bounds `382.43` ms slower p50 (`1713.89` vs `1331.46`). Rejected, null primary delta, latency regression; never reusable/promoted/synced/ship ([results](docs/design/continuous-openui-20260803-c3-results.md)) |
| Autotrain c4 (campaign `8c0b60dd-c2`) decode-timeout incomplete | 2 size-matched 1,755,764-param scratch checkpoints | `outputs/autoresearch/continuous-loop-20260803-continuous-openui-local-8c0b60dd-c2/runs/` (local, explicit no-sync) | Training fast (4.09s/8.15s) but AgentV smoke decode timed out 3/3 on both control (unconstrained "grammar levers off" diagnostic) and component-plan arms; measurement incomplete, no primary comparison. `repair_harness`/`retry_measurement` queued; never reusable/promoted/synced/ship ([results](docs/design/continuous-openui-20260803-c4-results.md)) |
