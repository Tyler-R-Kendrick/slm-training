...
| Autotrain 8c0b60dd-c3 control decode timeout | 2 size-matched scratch checkpoints | `outputs/autoresearch/continuous-loop-20260803-continuous-openui-local-8c0b60dd-c3/runs/` (local, explicit no-sync) | Control smoke eval incomplete (3/3 documents hit decode timeout); component-plan evaluated with `meaningful_program_rate=0.0`. Not comparable; soft failure, not a repeat blocker. Never reusable/promoted/synced/ship ([results](docs/design/autotrain-cycle-c3-component-plan-control-decode-timeout-20260803.md)) |
...
| Autotrain 8c0b60dd-c1 AgentV infra failure + preflight guard (fresh container) | 2 size-matched 1,681,794-param scratch checkpoints, byte-identical to the `j48f8u` row above | `outputs/autoresearch/continuous-loop-20260803-continuous-openui-local-8c0b60dd-c1/runs/` (local, explicit no-sync) | Same deterministic checkpoints, but this container never ran `scripts/setup_dev_env.sh` so `--ship-gates` crashed on a missing AgentV SDK; not scoreable. Fixed with `npm ci` and added `missing_dev_env_prerequisites()` to `scripts/run_autotrain_continuous.py` so future fresh containers fail fast instead of re-training both arms. Never reusable/promoted/synced/ship ([results](docs/design/autotrain-cycle-c1-agentv-missing-infra-preflight-20260803.md)) |
