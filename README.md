...
| --- | --- | --- | --- |
| Autotrain scheduled-loop `1e62ecf9` c5 component-edge quality-neutral | 2 size-matched 1,766,987-param CPU scratch checkpoints | `outputs/autoresearch/continuous-loop-20260804-continuous-openui-schedu-1e62ecf9-c5/runs/` (local, explicit no-sync) | Control/candidate tie exactly on `structural_similarity` (.043067 both); candidate ~9.8% faster (p50 16175→14592 ms) but zero quality signal. Reject, no champion **([results](docs/design/continuous-openui-scheduled-1e62ecf9-c5-results.md))** |
| Autotrain scheduled-loop `1e62ecf9` c4 component-plan quality-neutral | 2 size-matched 1,755,764-param CPU scratch checkpoints | `outputs/autoresearch/continuous-loop-20260804-continuous-openui-schedu-1e62ecf9-c4/runs/` (local, explicit no-sync) | Control/candidate tie exactly on `structural_similarity` (.416667 both); candidate is ~18.6% slower (p50 22500→26693 ms) with no quality gain. Reject, no champion **([results](docs/design/continuous-openui-scheduled-1e62ecf9-c4-results.md))** |
| Autotrain scheduled-loop `1e62ecf9` c3 schema self-heal, screening null | 2 size-matched 1,608,962-param CPU scratch checkpoints | `outputs/autoresearch/continuous-loop-20260804-continuous-openui-schedu-1e62ecf9-c3/runs/` (local, explicit no-sync) | Two predecessor cycles (c1, c2) hard-blocked with 0/5 valid hypotheses on a schema bug (`generate_batch_size` rejected by `ExperimentKnobs`); repaired in `1f28f6e` (executable-unblock, plain infra-fix PR). This cycle completed end to end but ties control on `structural_similarity` (.230833 both) — null, no champion **([results](docs/design/continuous-openui-scheduled-1e62ecf9-c3-results.md))** |
