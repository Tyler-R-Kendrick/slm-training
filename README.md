...
| --- | --- | --- | --- |
| Autotrain `continuous-openui-jkk2fb` c2 component-plan screen | 2 size-matched 1,755,764-param scratch checkpoints | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-jkk2fb-9f2e5830-c2/runs/` (local, explicit no-sync) | Control trained to completion but wall-timed-out before evaluation (no suite metrics); candidate smoke n=3 parse 1.0, meaningful 0.0, structure .0964, p50 2088.0 ms. Incomplete measurement, not a comparison; ship-gate publish incomplete (AgentV SDK unavailable). Neither checkpoint reusable, promoted, synced, or ship ([results](docs/design/continuous-openui-jkk2fb-c2-results.md)) |
| Autotrain `continuous-openui-jkk2fb` c1 bounds screen | 2 exactly matched 1,608,962-param scratch checkpoints | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-jkk2fb-9f2e5830-c1/runs/` (local, explicit no-sync) | Both 20-step CPU arms (`grammar_completion_bounds` off/on) are exact smoke n=3 quality ties (parse 1.0, meaningful 0.0, structure .0575, binder F1 .6333); treatment p50 is only latency-different (2469.22→2380.30 ms) with zero quality movement — not a positive tradeoff. Ship-gate publish incomplete (AgentV SDK unavailable this session). Rejected; neither checkpoint reusable, promoted, synced, or ship ([results](docs/design/continuous-openui-jkk2fb-c1-results.md)) |
