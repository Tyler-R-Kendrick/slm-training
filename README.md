...
| --- | --- | --- | --- |
| Autotrain continuous-openui-local-2 c4 component-edge champion-confirm attempt (infrastructure wall-timeout, unconfirmed) | 2 differently-trained scratch checkpoints (`component_edge_decode_weight` off vs `component-edge` head profile), `eval_suites=smoke,held_out` | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-local--c94ddb78-c4/runs/` (local, explicit no-sync) | First champion-confirmation cycle this lineage: attempted to confirm c3's driver-tagged POSITIVE `component-edge` efficiency-win (already reclassified NON_POSITIVE by loop policy) with a full ship-gate scoreboard. Training completed on both arms and `smoke` tied bit-identical again, but the `evaluate_model` stage was killed by the wall-time guard before `held_out` finished on either arm — no scoreboard written, `measurement_complete=false`. The reported held_out tie is a carried-forward smoke placeholder, not a real measurement. Non-positive (infrastructure wall-timeout); `retry_measurement` queued for next cycle; never promoted/synced/ship ([results](docs/design/continuous-openui-local-2-20260802-c4-results.md)) |
| Autotrain continuous-openui-local-2 c3 component-edge screen (latency-only tied null) | 2 differently-trained scratch checkpoints (`component_edge_decode_weight` off vs `component-edge` head profile) | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-local--c94ddb78-c3/runs/` (local, explicit no-sync) | Driver freely picked its own rank-1 next-run priority from c2, `component-edge`. eval_version picked automatically; both arms completed with zero decode timeouts. Training loss diverged as expected (lever fired), but every smoke quality metric came back bit-identical between arms; only latency moved (mixed direction: p50 down, p95 up). Driver's own classifier tagged `positive=true` via its latency/mpr efficiency-win rule; this doc reclassifies `NON_POSITIVE` per the loop's "pure latency blip is not positive" policy (all quality metrics tied, primary metric delta exactly 0.0) — no operational change either way since `stack_layer=false` under both readings. Gates fail closed (insufficient_n, quality thresholds). Non-positive (reclassified from driver's POSITIVE tag); never promoted/synced/ship ([results](docs/design/continuous-openui-local-2-20260802-c3-results.md)) |
| Autotrain continuous-openui-local-2 c2 component-plan screen (regression) | 2 differently-trained scratch checkpoints (`component_plan_loss_weight` 0.0 vs 1.0, seed 100002) | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-local--c94ddb78-c2/runs/` (local, explicit no-sync) | Driver freely picked its own rank-1 next-run priority from c1, `component-plan`. eval_version picked automatically; both arms completed with zero decode timeouts. Unlike c1's bit-identical null, this shows a genuine regression: structural_similarity 0.3267→0.0964 (Δ-0.2303), component_type_recall 0.1667→0.0833; training loss diverged as expected, confirming the lever fired. Mirrors the same lever's confirmed regression in the parked `continuous-openui-local` lineage's c5. Gates fail closed (insufficient_n, quality thresholds). Non-positive (confirmed regression, not merely null); never promoted/synced/ship ([results](docs/design/continuous-openui-local-2-20260802-c2-results.md)) |
| Autotrain continuous-openui-local-2 c1 grammar-bounds screen (fresh lineage, null) | 2 exactly matched 1,608,962-param scratch checkpoints | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-local--c94ddb78-c1/runs/` (local, explicit no-sync) | First cycle of a new loop lineage started because the prior `continuous-openui-local` lineage is correctly self-gated by its own unacknowledged `repair_harness` action (decode-compiler performance, tracked separately in PR #1307; out of scope here). Confirms the prior lineage's `_feasible_eval_version` harness fix works on a fresh cycle: eval_version picked automatically, both arms completed with zero decode timeouts. Driver's thrash rotation picked `bounds` (`grammar_completion_bounds`) from the untested lever bank. smoke n=3 quality ties (structural_similarity 0.0575=0.0575, binder F1 0.6333=0.6333); only latency differs (candidate slower, no quality change). Gates fail closed (insufficient_n, quality thresholds). Non-positive; never promoted/synced/ship ([results](docs/design/continuous-openui-local-2-20260802-c1-results.md)) |
| Autotrain continuous-openui-local c9 binder-topology screen at 2x steps (null) | 2 differently-trained scratch checkpoints (`binder_topology_loss_weight` 0.0 vs 0.25, fresh seed 100009, **steps escalated 2x to 40**) | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-local-8c0b60dd-c9/runs/` (local, explicit no-sync) | Diagnostic scale-investigation step first: no larger published smoke/held_out eval snapshot exists, but `--steps` had comfortable headroom in the wall cap, so this cycle doubled steps 20-22→40. Driver selected `binder-topology`, its rank-1 carryover recommendation. Both arms trained cleanly (`stopped_on=steps`; loss diverges 6.802424→6.833416, proving the escalation was real), but every decode-facing smoke metric is still bit-identical (structural_similarity 0.51=0.51; only latency differs). Primary delta exactly 0.0 — a fourth bit-identical null this session, the first at 2x steps: answers that escalating steps does not reveal the signal these levers produce; the bottleneck is eval `n`. Gates fail closed (insufficient_n, quality thresholds). Non-positive; binder-topology exhausted at this recipe, never promoted/synced/ship ([results](docs/design/continuous-openui-local-20260802-c9-results.md)) |
| Autotrain continuous-openui-local c8 component-inventory screen (null) | 2 differently-trained scratch checkpoints (`component_inventory_loss_weight` 0.0 vs 1.0, fresh seed 100008) | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-local-8c0b60dd-c8/runs/` (local, explicit no-sync) | Driver finally selected its own twice-recommended rank-1 hypothesis `component-inventory`, a genuinely untested-this-session arm. Full ship-gate scoreboard completes for both arms across smoke+held_out; every decode-facing quality metric is bit-identical on both suites (only latency differs slightly, candidate marginally slower). Primary delta (held_out.structural_similarity) exactly 0.0 — a third bit-identical null this session. Gates fail closed (insufficient_n, quality thresholds). Non-positive; component-inventory exhausted at this recipe, never promoted/synced/ship ([results](docs/design/continuous-openui-local-20260802-c8-results.md)) |
| Autotrain continuous-openui-local c7 component-plan re-screen (null) | 2 differently-trained scratch checkpoints (`component_plan_loss_weight` 0.0 vs 1.0, fresh seed 100007) | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-local-8c0b60dd-c7/runs/` (local, explicit no-sync) | Driver re-selected `component-plan` (c5's rejected lever) as a fresh pair, not a replay. Full ship-gate scoreboard completes for both arms; every decode-facing smoke metric is bit-identical (only latency differs, candidate faster this time). Primary delta exactly 0.0 — a genuine null at this seed, distinct from c5's confirmed regression; does not overturn it. Gates fail closed (insufficient_n, quality thresholds). Non-positive; component-plan re-exhausted at this recipe, never promoted/synced/ship ([results](docs/design/continuous-openui-local-20260802-c7-results.md)) |
| Autotrain continuous-openui-local c6 component-edge screen (null) | 2 differently-trained scratch checkpoints (`component_edge_loss_weight` 0.0 vs 1.0) | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-local-8c0b60dd-c6/runs/` (local, explicit no-sync) | Full ship-gate scoreboard completes for both arms; every decode-facing smoke metric is bit-identical between arms (only latency differs, candidate slower). Primary delta exactly 0.0 — a genuine null, not a regression. Gates fail closed (insufficient_n, quality thresholds). Non-positive; component-edge exhausted at this recipe, never promoted/synced/ship ([results](docs/design/continuous-openui-local-20260802-c6-results.md)) |
| Autotrain continuous-openui-local c5 component-plan frozen replay (rejected) | 2 exactly matched scratch checkpoints (evaluation-only replay of c3) | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-local-8c0b60dd-c5/runs/` (local, explicit no-sync) | Full ship-gate scoreboard now completes for both arms after fixing a frozen-replay harness bug (hyphenated arm-slug truncation). Confirmed regression: structure 0.2308→0.1725, binder F1 0.7333→0.6333, meaningful 0.3333→0.0. Gates fail closed (insufficient_n, quality thresholds). Non-positive; component-plan rejected for this recipe, never promoted/synced/ship ([results](docs/design/continuous-openui-local-20260802-c5-results.md)) |
| Autotrain continuous-openui-local c3 component-plan incomplete screen | 2 exactly matched 1,755,764-param scratch checkpoints | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-local-8c0b60dd-c3/runs/` (local, explicit no-sync) | Both 20-step CPU arms completed; partial smoke n=3 diagnostic shows structure/binder-F1/meaningful regressions but full ship-gate scoreboard never wrote for either arm — recurrence of the `NODE_OPTIONS` blocker, fixed locally by re-running with it unset. Superseded by the c5 frozen replay above, which completes the measurement; never promoted/synced/ship ([results](docs/design/continuous-openui-local-20260802-c3-results.md)) |
| Autotrain continuous-openui-local c2 grammar-bounds frozen replay | 2 exactly matched scratch checkpoints (frozen replay of c1) | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-local-8c0b60dd-c2/runs/` (local, explicit no-sync) | Full ship-gate scoreboard now completes for both arms after fixing the local `NODE_OPTIONS`/AgentV blocker. smoke n=3 quality ties confirmed (parse 1.0, meaningful 0.0, structure .0575); bounds p50 41.1ms faster but meaningful_program_rate stays 0.0. Gates fail closed (insufficient_n, quality thresholds). Non-positive, confirms c1; bounds exhausted for this recipe, never promoted/synced/ship ([results](docs/design/continuous-openui-local-20260802-c2-results.md)) |
| Autotrain continuous-openui-local c1 grammar-bounds screen | 2 exactly matched 1,608,962-param scratch checkpoints | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-local-8c0b60dd-c1/runs/` (local, explicit no-sync) | Both 20-step CPU arms tie on the partial smoke n=3 diagnostic (parse 1.0, meaningful 0.0, structure .0575); bounds p50 is 75.6ms faster but meaningful_program_rate stays 0.0, so latency alone is not positive. Full ship-gate scoreboard never wrote for either arm — blocked by this sandbox's `NODE_OPTIONS` breaking the AgentV `node` subprocess, fixed locally via `npm ci`. Superseded by the c2 frozen replay above, which confirms non-positive; never promoted/synced/ship ([results](docs/design/continuous-openui-local-20260802-c1-results.md)) |
