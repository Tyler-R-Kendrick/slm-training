...
| --- | --- | --- | --- |
| Autotrain c1743 component-plan latency win (positive) | 2 exactly matched 1,755,764-param scratch checkpoints | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-202607-98199209-c3/runs/` (local, explicit no-sync) | Both 22-step CPU arms are exact smoke n=3 quality ties: parse 1.0, meaning .3333, structure .23083, binder F1 .4889, reward .5493. `component-plan` p50 is 5.79% faster (2,539.57→2,392.58 ms), an `mpr_per_ms` efficiency gain of 6.14% (≥5% floor) with quality held — SDLC Phase A **positive**; champion candidate queued. Fixture-scale single-seed screening only; not promoted, synced, or ship ([results](docs/design/autotrain-cycle-1743-component-plan-latency-win.md)) |
| Autotrain c1742 grammar-completion-bounds repeat screen | 2 exactly matched 1,608,962-param scratch checkpoints | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-202607-98199209-c2/runs/` (local, explicit no-sync) | Both 22-step CPU arms are exact smoke n=3 quality ties: parse 1.0, meaning 0, structure .0964, binder F1 0, reward 0. `grammar_completion_bounds` p50 is 5.99% slower (1,126.16→1,193.63 ms), confirming the c1741 result on a second seed. Rejected; neither checkpoint reusable, promoted, synced, or ship ([results](docs/design/autotrain-cycle-1742-grammar-bounds-repeat-screen.md)) |
| Autotrain c1741 grammar-completion-bounds screen | 2 exactly matched 1,608,962-param scratch checkpoints | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-202607-98199209-c1/runs/` (local, explicit no-sync) | Both 23-step CPU arms are exact smoke n=3 quality ties: parse 1.0, meaning 0, structure .0575, binder F1 .6333, reward 0. `grammar_completion_bounds` p50 is 12.29% slower (1,278.91→1,436.09 ms). This session's checkout was missing the pinned AgentV SDK, fixed with `npm ci` before re-evaluating the identical frozen checkpoints to complete the measurement. Rejected; neither checkpoint reusable, promoted, synced, or ship ([results](docs/design/autotrain-cycle-1741-grammar-bounds-latency-regression.md)) |
