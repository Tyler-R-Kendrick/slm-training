...
| E619 `slot_contract_in_context` scoring-gap replay | `e619-slot-contract-in-context-scratch80-20260720/last.pt` | `outputs/runs/e619-slot-contract-in-context-scratch80-20260720/` (local) | Fresh 80-step CPU scratch loop on E530 (checkpoint SHA byte-identical to E616/E617), loss 26.5243; matched OOD `n=4` control-vs-`--slot-contract-in-context` eval moves `required_inventory_coverage` from `UNKNOWN` to a real `FAIL` on one record (coverage 0.75→1.0), all headline quality metrics unchanged, strict v2 stays 0.0 — not ship |
