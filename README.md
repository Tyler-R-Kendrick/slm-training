...
| --- | --- | --- | --- |
| Autotrain `continuous-openui-local` c4 component-edge (control decode-timeout) | 2 size-matched 1,766,987-param CPU scratch checkpoints | `outputs/autoresearch/continuous-loop-20260803-continuous-openui-local-8c0b60dd-c4/runs/` (local, explicit no-sync) | Candidate completed (structure `.4167`, binder F1 `.9524`); matched control hit 3 typed decode timeouts before any scoreboard. **Measurement incomplete; exact frozen replay queued; never promote/sync/ship** ([results](docs/design/autotrain-cycle-continuous-openui-local-c4-component-edge-control-timeout.md)) |
