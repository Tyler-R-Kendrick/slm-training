...
| --- | --- | --- | --- |
| Autotrain p33lme c4 component-edge measurement-incomplete screen | 2 exactly matched 1,766,987-param scratch checkpoints | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-p33lme-489d3aa7-c4/runs/` (local, explicit no-sync) | Both 21-step CPU arms trained to completion, but `evaluate_model --ship-gates` for the `smoke,held_out` promotion suite pair exceeded its stage wall-time budget before either arm wrote a scoreboard. Inconclusive; measurement incomplete, exact frozen replay only, never promoted/synced/ship ([results](docs/design/autotrain-cycle-p33lme-c4-component-edge-screen.md)) |
| Autotrain p33lme c3 component-plan aux-head screen | 2 exactly matched 1,755,764-param scratch checkpoints | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-p33lme-489d3aa7-c3/runs/` (local, explicit no-sync) | Both 20-step CPU arms are smoke n=3 complete (AgentV, `execution_errors=0`), but the candidate regresses structural_similarity (.2308→.1725), meaningful_program_rate (.3333→0), and binder F1 (.7333→.6333), and is 6.53% slower. Rejected; neither checkpoint reusable, promoted, synced, or ship ([results](docs/design/autotrain-cycle-p33lme-c3-component-plan-screen.md)) |
| Autotrain p33lme c1 AgentV NODE_OPTIONS repair screen | 2 exactly matched 1,608,962-param scratch checkpoints | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-p33lme-489d3aa7-c1/runs/` (local, explicit no-sync) | Both 20-step CPU arms trained and decoded smoke n=3 to completion, but `evaluate_model --ship-gates` failed closed before writing a scoreboard (AgentV SDK node child inherited a broken `NODE_OPTIONS`, exit 9); fixed in `a82eea405548cea5fcc486c3332823e2e3b008cf`. Inconclusive; exact frozen replay only, never promoted/synced/ship ([results](docs/design/autotrain-cycle-p33lme-c1-agentv-node-options-repair.md)) |
