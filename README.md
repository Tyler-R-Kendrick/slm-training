...
| E620 `slot_contract_in_context` live control (corrects E618/E619) | `e619-slot-contract-in-context-scratch80-20260720/last.pt` | `outputs/runs/e619-slot-contract-in-context-scratch80-20260720/` (local) | Fresh 80-step CPU scratch loop on E530 (checkpoint SHA byte-identical to E616/E617/E619), loss 26.5243; live matched OOD `n=4` control (flag genuinely unset) vs `--slot-contract-in-context` treatment moves `required_inventory_coverage` from `UNKNOWN` to a real `FAIL` on `ood_gallery_01` only (coverage 0.75→1.0), correcting E618's and the concurrent E619's "every/all 4 records" framing; headline quality metrics unchanged, strict v2 stays 0.0 — not ship |
