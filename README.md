...
| E615 object-frame schema-role slot bias | `e615_training_loop_twotower_scratch_20260720/last.pt` | `outputs/runs/e615_training_loop_twotower_scratch_20260720/` (local) | Fresh 8-step CPU scratch loop on E530, loss 37.9045; matched OOD `n=4` control-vs-treatment eval was run but byte-identical (0/4 parseable, checkpoint too undertrained) — not ship |
...
| E622 training-time decode-gate check (40-step) | `e622-training-time-decode-gate-scratch40-20260720/last.pt` | `outputs/runs/e622-training-time-decode-gate-scratch40-20260720/` (local) | Fresh 40-step CPU scratch loop on E530, loss 15.3967; E617's decode gate populates live during training's own periodic eval for the first time, but `train_model.py` exposed no weighted lever behind it (all 0.0) — not ship |
| E623 decode-weight CLI-gap control/treatment (40-step) | `e623-{control,treatment}-scratch40-20260720/last.pt` | `outputs/runs/e623-{control,treatment}-scratch40-20260720/` (local) | Control byte-reproduces E622; treatment exercises the newly-wired CLI decode-weight flags live for the first time — `ood` `n=4` `reward_score` 0.0→0.6748, single-seed — not ship |
| E624 multi-seed replication (40-step, seed 1/2) | `e624-{control,treatment}-scratch40-seed{1,2}-20260720/last.pt` | `outputs/runs/e624-{control,treatment}-scratch40-seed{1,2}-20260720/` (local) | Four checkpoints replicating E623 at seed 1/2; mixed result (seed 2 reproduces the positive delta, seed 1's already-strong control makes treatment flat-to-negative) — not ship |
