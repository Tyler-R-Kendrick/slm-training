...
| --- | --- | --- | --- |
| Autotrain c4 (20260803 loop) dual-arm decode timeout, unresolved | 2 size-matched 1,766,990-param scratch checkpoints | `outputs/autoresearch/continuous-loop-20260803-continuous-openui-202608-fa9f9cf7-c4/runs/` (local, explicit no-sync) | Both arms finalized 3/3 records inside a 24.0s decode timeout; no scoreboard. Matches a same-day dual-arm timeout in a different loop; root cause unresolved, no speculative repair attempted; never reusable/promoted/synced/ship ([results](docs/design/continuous-openui-20260803-c4-dual-arm-decode-timeout.md)) |
| Autotrain c3 (20260803 loop) component-plan quality regression | 2 size-matched 1,755,760-param scratch checkpoints | `outputs/autoresearch/continuous-loop-20260803-continuous-openui-202608-fa9f9cf7-c3/runs/` (local, explicit no-sync) | Component-plan regresses structure `.2308->.1725`, MPR `.3333->0`, binder F1 `.7333->.6333`; latency improves but does not offset the loss; rejected, never reusable/promoted/synced/ship ([results](docs/design/continuous-openui-20260803-c3-component-plan-regression.md)) |
| Autotrain c1 (20260803 loop) cold-sandbox AgentV infra failure | 2 size-matched 1,608,962-param scratch checkpoints (bit-identical to prior loop's c2) | `outputs/autoresearch/continuous-loop-20260803-continuous-openui-202608-fa9f9cf7-c1/runs/` (local, explicit no-sync) | Both arms trained (loss `22.6219` each) but `--ship-gates` eval crashed on a missing AgentV SDK before any scoreboard; repaired with an `ensure_agentv_available()` preflight in `run_cycle()` (commit `41dfe6d`); not scoreable, not a model result; never reusable/promoted/synced/ship ([results](docs/design/continuous-openui-20260803-c1-infra-failure.md)) |
