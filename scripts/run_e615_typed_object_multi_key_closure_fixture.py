...
2. A regression check that objects whose schema names *no* required
   ...
   e.g. ``Input.rules``) remain unaffected because they declare no required keys.
...
the real ``ImageGallery`` item schema's ``required_properties`` tuple from
...
def _probe_component_argument_path(tok: ChoiceTokenizer) -> dict[str, Any]:
    ...
    required_properties = state.frames[-1].required_properties if opened else ()
    close_id = tok.token_to_id["}"]
    src_id = tok.token_to_id["n:src"]
    allowed = state.allowed_ids(16)
    return {
        "object_opened": opened,
        "required_properties_derived": list(required_properties),
...
def _probe_component_argument_no_required(tok: ChoiceTokenizer) -> dict[str, Any]:
    ...
    opened = state.advance_id(tok.token_to_id["{"])
    required_properties = state.frames[-1].required_properties if opened else ("<unopened>",)
...
def _probe_multi_key_order(tok: ChoiceTokenizer, *, order: tuple[str, str]) -> dict[str, Any]:
    ...
    for name in order:
        ...
        assert state.advance_id(tok.sym_id(0))
        steps.append(
            {
                "filled": name,
...
def run_fixture() -> dict[str, Any]:
    ...
    return {
        ...
        "run_class": "fixture",
        "hypothesis": (
            "E614's schema-required-property closure generalizes beyond the single "
        ...
        "recipe": {
            ...
            "field": "required_properties tuple widened to (src, alt); no shipped "
