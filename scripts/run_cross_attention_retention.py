#!/usr/bin/env python3
"""Run or verify the zero-training SLM-218 retrospective."""
...
DEFAULT_JSON = "docs/design/iter-slm218-cross-attention-retention-20260723.json"
DEFAULT_MARKDOWN = "docs/design/iter-slm218-cross-attention-retention-20260723.md"
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = run_retrospective(root)
    payload = report.to_dict()
    expected_json = json.dumps(payload, indent=2, sort_keys=True) + "\n"
    expected_markdown = render_markdown(report)
    if args.check:
        if not json_path.is_file() or not markdown_path.is_file():
            print("missing committed SLM-218 result")
            return 1
        ...
        if markdown_path.read_text(encoding="utf-8") != expected_markdown:
            ...
            return 1
        print(f"{report.overall_verdict} {report.report_hash}")
        return 0
