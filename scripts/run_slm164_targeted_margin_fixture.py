#!/usr/bin/env python3
"""Run the SLM-164 (SDE1-02) confusion-targeted legal-sibling margin fixture.
...
_DESIGN_JSON = "docs/design/iter-slm164-targeted-margin-20260720.json"
_DESIGN_MD = "docs/design/iter-slm164-targeted-margin-20260720.md"
...
def _build_payload(
    ...
    if mode == "plan-only":
        payload: dict[str, Any] = {
            "schema": "Slm164TargetedMarginManifestV1",
    ...
    report = run_fixture_campaign(
        arms=arms,
        run_id=f"slm164-targeted-margin-{_today_yyyymmdd()}",
        output_dir=output_dir,
        seeds=seeds,
        margin=margin,
    )
    payload = report.to_dict()
    command = "python -m scripts.run_slm164_targeted_margin_fixture --mode fixture"
    return payload, command
...
def _build_markdown(payload: dict[str, Any], command: str) -> str:
    ...
    if status == "plan_only":
        arms = [TargetedMarginArm.from_dict(a) for a in payload["arms"]]
        lines = [
            "# SLM-164 (SDE1-02): Confusion-targeted legal-sibling contrast margin plan",
            "",
            ...
            "`iter-slm164-targeted-margin-20260720.json`"
            "](iter-slm164-targeted-margin-20260720.json)",
            "",
            "This is a plan-only manifest. The targeted-margin arms, metrics, "
    ...
    report = TargetedMarginReport.from_dict(payload)
    return render_markdown(report)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="SLM-164 SDE1-02 confusion-targeted legal-sibling margin fixture",
        exit_on_error=False,
    )
    parser.add_argument(
        "--mode",
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        help="Directory for run artifacts (default: outputs/runs/slm164-targeted-margin-<YYYYMMDD>)",
    )
    parser.add_argument(
        "--seeds",
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Margin value for the fixture contrast loss (default: 1.0).",
    )
    try:
    ...
    output_dir = (
        args.output_dir
        or Path(f"outputs/runs/slm164-targeted-margin-{_today_yyyymmdd()}")
    )
    ...
    payload["schema"] = "Slm164TargetedMarginReportV1"
    ...
    run_json = output_dir / "slm164_targeted_margin_report.json"
