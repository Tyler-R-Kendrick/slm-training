#!/usr/bin/env python3
"""RESEARCH-07 / SLM-565: run default-off VSS Alethe/SMT pilot.
    ...
    PYTHONPATH=src uv run python -m scripts.run_research_07_vss_alethe --write-lock
"""
...
from slm_training.harnesses.experiments.research_07_vss_alethe import (
    ENABLE_FLAG,
    EVIDENCE_JSON,
    LOCK_RELPATH,
    load_campaign_lock,
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.json or not args.write:
        ...
        print(
            ...
            f"agreement={result.get('primary_value')} "
            f"disagreements={result.get('witness_disagreement_count')} "
            f"fake_refutations={result.get('reconstruction_failure_as_refutation_count')}"
        )
    return 0
