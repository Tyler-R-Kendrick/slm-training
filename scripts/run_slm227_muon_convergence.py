#!/usr/bin/env python3
"""Run the SLM-227 (NCS2-04) Muon/AdamW convergence-direction sweep.

Extends SLM-222's 2-step single-record optimizer-wiring smoke test to a
multi-seed, multi-step, multi-record CPU-only comparison and asks whether the
Muon/AdamW hybrid's final training loss moves in a consistent direction
relative to plain AdamW.

Examples:
  python -m scripts.run_slm227_muon_convergence --mode plan-only
  python -m scripts.run_slm227_muon_convergence --mode fixture
  python -m scripts.run_slm227_muon_convergence --mode fixture \
      --steps 40 --n-records 4 --seeds 0 1 2 3 4
"""

from __future__ import annotations

import argparse
import json
import sys
from datetime import datetime, timezone
from pathlib import Path
from typing import Any

from slm_training.harnesses.experiments.slm227_muon_convergence import (
    DEFAULT_N_RECORDS,
    DEFAULT_SEEDS,
    DEFAULT_STEPS,
    EXPERIMENT_ID,
    MATRIX_SET,
    MATRIX_VERSION,
    MuonConvergenceReport,
    render_markdown,
    run_muon_convergence_sweep,
)
from slm_training.versioning import build_version_stamp

_DESIGN_JSON = "docs/design/iter-slm227-muon-convergence-20260721.json"
_DESIGN_MD = "docs/design/iter-slm227-muon-convergence-20260721.md"


def _now() -> str:
    return datetime.now(timezone.utc).isoformat().replace("+00:00", "Z")


def _today_yyyymmdd() -> str:
    return datetime.now(timezone.utc).strftime("%Y%m%d")


def _build_plan_only_payload(steps: int, n_records: int, seeds: tuple[int, ...]) -> dict[str, Any]:
    return {
        "schema": "MuonConvergenceReportV1",
        "matrix_set": MATRIX_SET,
        "matrix_version": MATRIX_VERSION,
        "experiment_id": EXPERIMENT_ID,
        "run_id": f"{EXPERIMENT_ID}-{_today_yyyymmdd()}",
        "status": "plan_only",
        "claim_class": "wiring",
        "hypothesis": (
            "At matched initialization, learning rate, data, and step budget, the "
            "Muon/AdamW hybrid optimizer's final training loss moves in a consistent "
            "direction relative to plain AdamW across seeds."
        ),
        "falsifier": (
            "The Muon arm's final loss is not consistently lower (or higher) than "
            "AdamW's across the swept seeds, or either arm produces a non-finite "
            "loss at any seed."
        ),
        "steps": steps,
        "n_records": n_records,
        "seeds": list(seeds),
        "comparisons": [],
        "muon_wins": 0,
        "adamw_wins": 0,
        "ties": 0,
        "unstable_seeds": 0,
        "mean_delta": None,
        "stdev_delta": None,
        "all_finite": True,
        "disposition": "inconclusive",
        "disposition_rationale": (
            "Plan-only manifest; run `python -m scripts.run_slm227_muon_convergence "
            "--mode fixture` to execute."
        ),
        "honest_caveats": [
            "Plan-only: no seed was evaluated.",
        ],
        "version_stamp": build_version_stamp(
            "harness.experiments",
            "harness.experiments.slm227_muon_convergence",
            "harness.experiments.slm222_muon_baseline",
            "harness.model_build.train",
            "model.twotower",
        ),
        "timestamp": _now(),
    }


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="SLM-227 NCS2-04 Muon/AdamW convergence-direction sweep",
        exit_on_error=False,
    )
    parser.add_argument(
        "--mode",
        choices={"plan-only", "fixture"},
        default="fixture",
        help="Run mode (default: fixture).",
    )
    parser.add_argument(
        "--steps",
        type=int,
        default=DEFAULT_STEPS,
        help=f"Training steps per arm (default: {DEFAULT_STEPS}).",
    )
    parser.add_argument(
        "--n-records",
        type=int,
        default=DEFAULT_N_RECORDS,
        help=f"Number of synthetic train records (default: {DEFAULT_N_RECORDS}).",
    )
    parser.add_argument(
        "--batch-size",
        type=int,
        default=2,
        help="Batch size for both arms (default: 2).",
    )
    parser.add_argument(
        "--seeds",
        type=int,
        nargs="+",
        default=list(DEFAULT_SEEDS),
        help=f"Seeds to sweep (default: {list(DEFAULT_SEEDS)}).",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        help="Directory for run artifacts.",
    )
    parser.add_argument(
        "--write-design-docs",
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Write design docs in fixture mode (default: True).",
    )
    parser.add_argument("--design-json", type=Path, default=None)
    parser.add_argument("--design-md", type=Path, default=None)

    try:
        args = parser.parse_args(argv)
    except (argparse.ArgumentError, SystemExit):
        return 2

    args.output_dir = args.output_dir or Path(
        f"outputs/runs/slm227-muon-convergence-{_today_yyyymmdd()}"
    )
    args.output_dir.mkdir(parents=True, exist_ok=True)

    if args.mode == "plan-only":
        payload = _build_plan_only_payload(args.steps, args.n_records, tuple(args.seeds))
    else:
        report = run_muon_convergence_sweep(
            steps=args.steps,
            n_records=args.n_records,
            batch_size=args.batch_size,
            seeds=tuple(args.seeds),
            run_id=f"{EXPERIMENT_ID}-{_today_yyyymmdd()}",
        )
        payload = report.to_dict()

    run_json = args.output_dir / "slm227_muon_convergence_report.json"
    run_json.write_text(
        json.dumps(payload, indent=2, sort_keys=True, default=str) + "\n",
        encoding="utf-8",
    )

    if args.mode == "fixture" and args.write_design_docs:
        root = Path(__file__).resolve().parents[1]
        json_path = args.design_json or root / _DESIGN_JSON
        md_path = args.design_md or root / _DESIGN_MD
        json_path.parent.mkdir(parents=True, exist_ok=True)
        md_path.parent.mkdir(parents=True, exist_ok=True)
        json_path.write_text(
            json.dumps(payload, indent=2, sort_keys=True, default=str) + "\n",
            encoding="utf-8",
        )
        md_path.write_text(
            render_markdown(MuonConvergenceReport.from_dict(payload)),
            encoding="utf-8",
        )

    print(str(run_json))
    return 0


if __name__ == "__main__":
    sys.exit(main())
