...
# SLM-431 (LAR0-06): additive, default-off powered-rerun decision surface,
# structurally mirroring SLM-421's power_rule addition to
# scripts/run_slm138_recursive_denoiser_fixture.py. When --min-pass-rate is
# supplied, the value decision moves from the original n=2 per-example Wilson
# screen to a per-seed pass/fail Wilson interval over seeds DISJOINT from the
# original [0, 1]; the corpus, matched arms, do-no-harm commit rule, safety
# gate, and reachability gate stay byte-identical. power_rule=None (the
# default) preserves the original 2-seed behavior byte-for-byte.
DISPOSITION_UNDERPOWERED = "inconclusive_underpowered"

POWER_PREREGISTERED = {
    "seed_pass_rule": (
        "a seed passes iff ar_repair_improved shows >=1 paired improvement "
        "over ar_only (AR invalid, arm valid) with 0 damages (AR valid, arm "
        "invalid) on that seed's frozen SLM-155 eval decisions"
    ),
    "disposition_rule": (
        "repair_positive iff safety AND reachability gates pass AND the "
        "Wilson 95% lower bound of the per-seed pass rate clears "
        "min_pass_rate; repair_negative iff safety fails OR the Wilson UPPER "
        "bound falls below min_pass_rate (pass rate ruled out); else "
        "inconclusive_underpowered"
    ),
    "seed_disjointness": (
        "powered-rerun seeds must be disjoint from SLM-317's original "
        "[0, 1] (mirrors SLM-421's 2..21)"
    ),
}
...
def power_disposition(
    *,
    ...
    """Preregistered Wilson-interval power decision (SLM-431, default-off).

    Locked before any new seed's results are read. Returns the disposition
    and the Wilson interval of the per-seed pass rate. Mirrors SLM-421's
    rule: positive iff the interval lower bound clears ``min_pass_rate``
    (with safety AND reachability passing), negative iff safety fails or the
    interval upper bound falls below it (the pass rate is ruled out), else
    ``inconclusive_underpowered``.
    """
    ...
    interval = wilson_interval(
        passed_seeds, n_seeds, confidence_level=confidence_level
    )
    if not safety_pass or (
        interval["high"] is not None and interval["high"] < min_pass_rate
    ):
        disposition = DISPOSITION_NEGATIVE
    elif (
        reachability_pass
        and interval["low"] is not None
        and interval["low"] >= min_pass_rate
    ):
        disposition = DISPOSITION_POSITIVE
    else:
        disposition = DISPOSITION_UNDERPOWERED
    return disposition, interval
...
def run_screen(
    *,
    ...
    per_seed_results: list[dict[str, Any]] | None = None
    if power_rule is None:
        ...
        # SLM-431: per-seed pass/fail under the preregistered improvement
        # rule (>=1 improvement, 0 damages per seed), then the Wilson power
        # decision. Safety and reachability gates stay independent.
        per_seed_results = []
        for seed in seeds:
            seed_ar = [o for o in ar_outcomes if o.seed == seed]
            seed_arm = [o for o in by_arm["ar_repair_improved"] if o.seed == seed]
            seed_paired = paired_validity_table(seed_ar, seed_arm)
            seed_improvements = seed_paired["a_invalid_b_valid"]
            seed_damages = seed_paired["a_valid_b_invalid"]
            per_seed_results.append(
                {
                    "seed": seed,
                    "improvements": seed_improvements,
                    "damages": seed_damages,
                    "paired": seed_paired,
                    "pass": seed_improvements >= 1 and seed_damages == 0,
                }
            )
        passed_seeds = sum(row["pass"] for row in per_seed_results)
        disposition, power_interval = power_disposition(
            passed_seeds=passed_seeds,
            n_seeds=len(seeds),
            min_pass_rate=power_rule["min_pass_rate"],
            safety_pass=safety_pass,
            reachability_pass=reach_pass,
            confidence_level=float(power_rule.get("confidence_level", 0.95)),
        )
        power_evidence = {
            "min_pass_rate": float(power_rule["min_pass_rate"]),
            "seed_pass_rule": POWER_PREREGISTERED["seed_pass_rule"],
            "disposition_rule": POWER_PREREGISTERED["disposition_rule"],
            "passed_seed_count": passed_seeds,
            "seed_count": len(seeds),
            "interval": {"method": "wilson_score", **power_interval},
        }
    ...
    if power_rule is None:
        ...
        lar3_statement = {
            DISPOSITION_POSITIVE: (
                "LAR3 (learned repair integration): BOTH PR #853-#856 reopening "
                "conditions are now met (SLM-421's recurrence-health condition "
                "and this powered do-no-harm repair value gate). This artifact "
                "does NOT reopen LAR3 — a separate, explicit LAR3-reopening "
                "decision issue is required next."
            ),
            DISPOSITION_NEGATIVE: (
                "LAR3 (learned repair integration) stays CLOSED: the powered "
                "rerun damaged valid AR programs or ruled out the preregistered "
                "minimum per-seed pass rate; the second reopening condition is "
                "not met."
            ),
            DISPOSITION_UNDERPOWERED: (
                "LAR3 (learned repair integration) remains unresolved: at this "
                "seed count the Wilson interval neither clears nor rules out "
                "the preregistered pass rate; a higher-powered rerun (within "
                "MAX_RUN_MINUTES) is required before the second reopening "
                "condition can resolve."
            ),
        }[disposition]
    ...
    if power_evidence is not None:
        payload["issue"] = "SLM-431"
        payload["power_rerun_of"] = "SLM-317"
        payload["power_rule"] = power_evidence
        payload["per_seed_results"] = per_seed_results
        payload["honesty"] = payload["honesty"] + (
            " SLM-431 powered rerun: only the seed count and the value "
            "decision rule changed (per-seed pass/fail + Wilson interval, "
            "preregistered and locked before any new seed was read); the "
            "corpus, matched arms, do-no-harm commit rule, safety gate, and "
            "reachability gate are byte-identical to SLM-317. New seeds are "
            "disjoint from SLM-317's original [0, 1]."
        )
    return payload
...
def render_markdown(payload: dict) -> str:
    ...
    if power is not None:
        ...
        lines += [
            ...
            "",
            f"- seed pass rule: {power['seed_pass_rule']}",
            f"- disposition rule: {power['disposition_rule']}",
            f"- seed count: `{power['seed_count']}` "
            f"(disjoint from SLM-317's original [0, 1])",
            f"- observed passing seeds: `{power['passed_seed_count']}`",
            f"- observed pass rate: `{interval['estimate']:.4f}` "
            f"(Wilson 95% CI [`{interval['low']:.4f}`, `{interval['high']:.4f}`], "
            f"n=`{interval['n']}`) vs preregistered min pass rate "
            f"`{power['min_pass_rate']}`",
            "",
            "| seed | improvements | damages | pass |",
            "| ---: | ---: | ---: | --- |",
        ]
        for row in payload["per_seed_results"]:
            lines.append(
                f"| {row['seed']} | {row['improvements']} | {row['damages']} | "
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "SLM-431 powered-rerun mode (default off). Enables a "
            "Wilson-interval decision rule over the per-seed pass rate (see "
            "slm_training.evals.power_protocol.wilson_interval) instead of "
            "the original 2-seed per-example screen: repair_positive iff the "
            "interval lower bound clears this rate (with safety AND "
            "reachability passing), repair_negative iff the upper bound "
            "falls below it, else inconclusive_underpowered. Required for "
            "any --seeds other than the original [0, 1]; new seeds must be "
            "disjoint from [0, 1]."
        ),
    )
    ...
    if args.seeds != [0, 1] and args.min_pass_rate is None:
        parser.error(
            "--seeds other than the original [0, 1] requires --min-pass-rate "
            "(a raw per-example screen is not a meaningful decision rule at "
    ...
    if power_rule is not None:
        # Never clobber the frozen SLM-317 20260724 record: powered runs get
        # their own dated stem (mirrors SLM-421's powered-rerun stem switch).
        today = _today_slug()
        if json_out == DEFAULT_JSON_OUT:
            json_out = Path(
                f"docs/design/iter-slm317-repair-hybrid-powered-rerun-{today}.json"
            )
        if md_out == DEFAULT_MD_OUT:
            md_out = Path(
                f"docs/design/iter-slm317-repair-hybrid-powered-rerun-{today}.md"
            )
        if decisions_out == DEFAULT_DECISIONS_OUT:
            decisions_out = Path(
                f"outputs/slm317/commit_decisions_powered_{today}.jsonl"
            )
