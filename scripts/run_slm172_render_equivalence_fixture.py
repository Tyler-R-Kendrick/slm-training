#!/usr/bin/env python3
"""Run the SLM-172 (SDE2-05) render-equivalence fixture.
...
_DESIGN_JSON = "docs/design/iter-slm172-render-equivalence-20260720.json"
_DESIGN_MD = "docs/design/iter-slm172-render-equivalence-20260720.md"
...
def _build_payload(
    ...
    if mode == "plan-only":
        payload: dict[str, Any] = {
            "schema": "Slm172RenderEquivalenceManifestV1",
    ...
    report = run_fixture_campaign(
        cells=cells,
        run_id=f"slm172-render-equivalence-{_today_yyyymmdd()}",
        output_dir=output_dir,
        seeds=seeds,
    )
    payload = report.to_dict()
    command = "python -m scripts.run_slm172_render_equivalence_fixture --mode fixture"
    return payload, command
...
def _build_markdown(payload: dict[str, Any], command: str) -> str:
    ...
    if status == "plan_only":
        cells = [RenderEquivalenceArm.from_dict(c) for c in payload["cells"]]
        lines = [
            "# SLM-172 (SDE2-05): render-equivalence fixture plan",
            "",
            ...
            "`iter-slm172-render-equivalence-20260720.json`"
            "](iter-slm172-render-equivalence-20260720.json)",
            "",
            "This is a plan-only manifest. The render-equivalence arms, tiered metric, "
    ...
    report = RenderEquivalenceReport.from_dict(payload)
    return render_markdown(report)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="SLM-172 SDE2-05 render-equivalence fixture",
        exit_on_error=False,
    )
    parser.add_argument(
        "--mode",
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        help="Directory for run artifacts (default: outputs/runs/slm172-render-equivalence-<YYYYMMDD>)",
    )
    parser.add_argument(
        "--seeds",
    ...
    output_dir = (
        args.output_dir
        or Path(f"outputs/runs/slm172-render-equivalence-{_today_yyyymmdd()}")
    )
    ...
    payload["schema"] = "Slm172RenderEquivalenceReportV1"
    ...
    run_json = output_dir / "slm172_render_equivalence_report.json"
