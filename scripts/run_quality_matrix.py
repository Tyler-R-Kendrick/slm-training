...
def _train_cfg(exp: Experiment, args: argparse.Namespace) -> ModelBuildConfig:
    return ModelBuildConfig(
        ...
        slot_component_class_balance_power=float(
            getattr(exp, "slot_component_class_balance_power", 0.0) or 0.0
        ),
        slot_component_decode_weight=float(
            getattr(exp, "slot_component_decode_weight", 0.0) or 0.0
        ),
        slot_component_prompt_context=bool(
            getattr(exp, "slot_component_prompt_context", True)
        ),
        slot_component_next_context=bool(
            getattr(exp, "slot_component_next_context", False)
        ),
        slot_component_pair_interaction=bool(
            getattr(exp, "slot_component_pair_interaction", False)
        ),
        ...
        slot_component_content_arity=bool(
            getattr(exp, "slot_component_content_arity", False)
        ),
