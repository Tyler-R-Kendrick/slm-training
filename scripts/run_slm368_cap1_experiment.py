#!/usr/bin/env python3
"""Run the SLM-368 (DSH2-08) matched CAP1 grounding experiment.
...
Example:
  python -m scripts.run_slm368_cap1_experiment --mode plan-only
  python -m scripts.run_slm368_cap1_experiment --mode fixture --arms NL_NO_SCHEMA SCHEMA_UNFILTERED
  python -m scripts.run_slm368_cap1_experiment --mode fixture --arms SCHEMA_FILTERED_SINGLE SCHEMA_FILTERED_MULTI
...
_DESIGN_JSON = "docs/design/iter-slm368-cap1-experiment-20260725.json"
_DESIGN_MD = "docs/design/iter-slm368-cap1-experiment-20260725.md"
_STAMP_COMPONENTS = (
    "harness.experiments",
    "harness.experiments.slm368_cap1_experiment",
    "harness.experiments.slm367_two_pack_freeze",
    "harness.train_data",
)
...
def _agentv_diagnostic(run_dir: Path, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        cases = [
            {
                "id": "cap1-certificate-decision",
                "criteria": (
                    "CERT_CAP1 decision is reported with its stop-rule codes; "
                ...
                "assertions": [
                    {
                        "id": "cap1-certificate-decision:status_declared",
        ...
        result = publish_agentv_evaluation(
            run_dir,
            name="slm368-cap1-experiment",
            claim=(
                "CAP1 grounding experiment diagnostic: fixture-scale CERT_CAP1 "
...
def _write_final(payload: dict[str, Any], output_dir: Path, command: str) -> Path:
    ...
    report_path = output_dir / "slm368_cap1_experiment_report.json"
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="SLM-368 DSH2-08 matched CAP1 grounding experiment",
        exit_on_error=False,
    )
    parser.add_argument(
        "--mode",
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        help="Run artifact directory (default: outputs/experiments/slm368-cap1)",
    )
    ...
    output_dir = args.output_dir or Path("outputs/experiments/slm368-cap1")
    output_dir.mkdir(parents=True, exist_ok=True)
    command = f"python -m scripts.run_slm368_cap1_experiment --mode {args.mode}"
    ...
    if args.mode == "plan-only":
        ...
        report_path = output_dir / "slm368_cap1_experiment_plan.json"
    ...
    if args.mode == "fixture":
        ...
        for arm in args.arms:
            shard = run_arm_shard(context, arm, output_dir=output_dir)
