...
Example:
  python -m scripts.run_slm491_external_blocked_closeout
  python -m scripts.run_slm491_external_blocked_closeout --output-dir outputs/runs/slm491
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("outputs/runs/slm491_external_blocked_closeout"),
    )
    ...
    run_json = args.output_dir / "slm491_external_blocked_closeout.json"
    ...
    print(f"status={payload.get('status')} kappa={payload.get('evaluator_human_agreement_kappa')}")
    return 0
