...
| --- | --- | --- | --- | --- |
| 2026-08-02 | `c20260802-continuous-openui-local-8c0b60dd-c2-{control,component-plan}` | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-local-8c0b60dd-c2/runs/` (local, explicit no-sync) | Inconclusive: component-plan arm completed (structure .0964, gate reject); control's `evaluate_model --ship-gates` hit the harness wall-time cap before a scoreboard was produced | CPU `wf_smoke_v2`, 22 steps; matched 1,755,764-param arms. Component-plan SHA `68f1ce95…f903bdf`; control SHA `417b3ce8…423c274cc`. No matched comparison exists; driver queued `retry_measurement` to replay the identical frozen arms; neither checkpoint reusable/promotable/ship ([results](design/autotrain-cycle-8c0b60dd-c2-component-plan-timeout.md)) |
| 2026-08-02 | `c20260802-continuous-openui-local-8c0b60dd-c1-{control,bounds}` | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-local-8c0b60dd-c1/runs/` (local, explicit no-sync) | Screen: exact smoke n=3 quality ties (parse 1.0, meaning 0, structure .0575, recall 0); bounds p50 1,846.50→1,771.30 ms (-4.08%), train wall 4.974→4.060 s; AgentV complete, gates fail | CPU `wf_smoke_v2`, seed 100001, batch 2; exactly matched 1,608,962-param arms. Control SHA `c2ba0527…f7eda2`; bounds SHA `b040e8f4…8f8e155`. First cycle of a fresh local continuous-loop identity (`continuous-openui-local`); prior `202607-98199209` lineage state (through c1759) is not recoverable in this ephemeral checkout (`outputs/` is gitignored). Quality null, latency-only gain; neither checkpoint reusable/promotable/ship ([results](design/autotrain-cycle-8c0b60dd-c1-grammar-bounds-screen.md)) |
