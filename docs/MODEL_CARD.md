...
| --- | --- | --- | --- | --- |
| Autotrain c1726 held_out wall timeout | `c20260802-continuous-openui-local-8c0b60dd-c4-{control,steps}` | CPU scratch TwoTower, `wf_smoke_v2`, lexer output, 21 vs 42 steps, batch 2, 1,608,962 trainable params | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-local-8c0b60dd-c4/runs/` (local, explicit no-sync) | Promotion-role cycle requesting `smoke,held_out`; both arms hit the repository `MAX_RUN_MINUTES=3` wall cap before `held_out` finished. Control captured zero metrics. `steps` (42 steps) completed smoke only: parse 1.0, binder F1 0.8222, meaningful 0.3333 (first non-zero meaningful smoke result in this loop), structure 0.51 — promising but the declared primary metric `held_out.structural_similarity` never materialized. **Incomplete; never reuse, promote, sync, or ship; frozen replay required** ([results](design/autotrain-cycle-1726-held-out-wall-timeout.md)) |
| Autotrain c1725 rotated lever | `c20260802-continuous-openui-local-8c0b60dd-c3-{control,both}` | CPU scratch TwoTower, `wf_smoke_v2`, lexer output, 20 steps, seed 100001, batch 2, 1,608,962 trainable params | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-local-8c0b60dd-c3/runs/` (local, explicit no-sync) | Both arms AgentV smoke n=3: parse 1.0, binder F1 0.7222, meaningful 0.0, structure 0.1725; honest gates failed (insufficient_n, missing held_out/adversarial/ood/rico_held). `both` carried a genuinely distinct config hash from control (first real lever test in this loop) but landed on identical smoke-scale metrics — underpowered, not evidence of an inert lever. **Not promotable or ship** ([results](design/autotrain-cycle-1725-rotated-lever-null-delta.md)) |
