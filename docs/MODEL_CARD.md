...
| --- | --- | --- | --- | --- |
| Autotrain 1pse54 c4 promotion decode timeout | `c20260802-continuous-openui-1pse54-c1f3ee08-c4-{control,component-plan}` | CPU scratch TwoTower, promotion role, 21 steps, seed 100001, batch 2; both 1,755,764 params (component-plan head prebuilt) | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-1pse54-c1f3ee08-c4/runs/` (local, explicit no-sync) | Both trains completed but every document (3/3 smoke, 5/5 held_out) on both arms hit the decode timeout; AgentV itself ran cleanly (0 execution errors). First occurrence of this signal, not yet a hard block. **Infrastructure only; never reuse/promote/sync/ship** ([results](design/autotrain-cycle-1pse54-c4-promotion-decode-timeout.md)) |
| Autotrain 1pse54 c2 AgentV NODE_OPTIONS block | `c20260802-continuous-openui-1pse54-c1f3ee08-c2-{control,bounds}` | CPU scratch TwoTower, 21 steps, seed 100001, batch 2; both 1,608,962 params | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-1pse54-c1f3ee08-c2/runs/` (local, explicit no-sync) | Both trains completed; evaluation blocked on both arms by an unsanitized NODE_OPTIONS crashing the AgentV node subprocess. Repaired in commit `46fecfc` (evals.agentv v6→v7); frozen replay pending. **Infrastructure only; never reuse/promote/sync/ship** ([results](design/autotrain-cycle-1pse54-c2-agentv-node-options-block.md)) |
