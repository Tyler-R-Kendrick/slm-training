...
| --- | --- | --- | --- | --- |
| Autotrain continuous-openui-local-2 c4 component-edge champion-confirm attempt (infrastructure wall-timeout, unconfirmed) | `c20260802-continuous-openui-local--c94ddb78-c4-{control,confirm}` | CPU scratch TwoTower, `wf_smoke_v2`, lexer output, 20 steps (recorded 20), record_count=101; both arms 1,766,987 trainable params | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-local--c94ddb78-c4/runs/` (local, explicit no-sync) | Fourth cycle of `continuous-openui-local-2`, first as a champion-confirmation (`cycle_role=promotion`, `cycle_intent=confirm`): c3's driver-tagged POSITIVE `component-edge` efficiency-win (already reclassified `NON_POSITIVE` by this lineage's own quality policy) was auto-enqueued to `champion_queue.jsonl` and this cycle attempted to confirm it with a full `--ship-gates` scoreboard (`eval_suites=smoke,held_out`) against a matched control. Training completed cleanly on both arms (last_loss 13.395→14.686, steps_actual=20) and the `smoke` suite (n=3) completed and tied again bit-identical between arms exactly as in every prior cycle (structural_similarity 0.4167=0.4167, parse 1.0=1.0, binder F1 0.9524=0.9524; latency confirm slower by 1206.89ms). But the `held_out` suite never finished on either arm — the `evaluate_model` stage was killed by the harness's own wall-time guard mid-suite on both, so no scoreboard was ever written (`measurement_complete=false`). The `held_out.structural_similarity` tie the driver reports (0.4167=0.4167) is the completed smoke value carried forward as a placeholder, not a genuine held_out measurement — read this cycle as "no comparison happened," not "a second tied null." **Non-positive (infrastructure wall-timeout, not a model result); no repair_harness signal; `retry_measurement` queued to replay the identical frozen manifest next cycle; never promote, sync, or ship** ([results](design/continuous-openui-local-2-20260802-c4-results.md)) |
| Autotrain continuous-openui-local-2 c3 component-edge screen (latency-only tied null) | `c20260802-continuous-openui-local--c94ddb78-c3-{control,component-edge}` | CPU scratch TwoTower, `wf_smoke_v2`, lexer output, 20 steps (recorded 20), record_count=101; both arms 1,766,987 trainable params | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-local--c94ddb78-c3/runs/` (local, explicit no-sync) | Third cycle of `continuous-openui-local-2`; the driver was left free to pick and it selected its own rank-1 next-run priority from cycle 2, `component-edge` (`component_edge_decode_weight`), no override applied. `eval_version=e938_role_safe_all_targets_v2` (smoke, n=3) again picked automatically; **both arms completed with zero decode timeouts**. Training loss diverged as expected (last_loss 15.298→16.483, both steps_actual=20), confirming the lever fired, but every smoke quality metric came back bit-identical between arms (parse, meaningful_program_rate, structural_similarity, binder_reference_f1, ast/canonical_beq_rate, reward_score, component_type_recall, placeholder_fidelity all tied); only latency moved (p50 -448.99ms/-12.0%, p95 +115.47ms, mixed direction). The driver's own classifier tagged this `positive=true` via its `mpr_per_ms` efficiency-win rule (quality held, gain_fraction 0.1367≥0.05), but this doc reclassifies it `NON_POSITIVE` per the loop's own "pure latency blip is not positive" policy, since literally every quality metric is tied and the primary metric moved by exactly 0.0 — operationally unchanged, since the driver's own `stack_layer` was already `false` either way. Gates fail closed on `insufficient_n` (3<20) and quality thresholds. **Non-positive (latency-only tied null, reclassified from the driver's own POSITIVE tag); no repair_harness signal; never promote, sync, or ship** ([results](design/continuous-openui-local-2-20260802-c3-results.md)) |
| Autotrain continuous-openui-local-2 c2 component-plan screen (regression) | `c20260802-continuous-openui-local--c94ddb78-c2-{control,component-plan}` | CPU scratch TwoTower, `wf_smoke_v2`, lexer output, 20 steps (recorded 22), seed 100002; both arms 1,755,764 trainable params | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-local--c94ddb78-c2/runs/` (local, explicit no-sync) | Second cycle of `continuous-openui-local-2`; the driver was left free to pick and it selected its own rank-1 next-run priority from cycle 1, `component-plan` (`component_plan_loss_weight`), no override applied. `eval_version=e938_role_safe_all_targets_v2` (smoke, n=3) again picked automatically; **both arms completed with zero decode timeouts**. Unlike cycle 1's bit-identical null, this cycle shows a genuine regression: structural_similarity fell 0.3267→0.0964 (Δ-0.2303), component_type_recall fell 0.1667→0.0833; training loss diverged as expected (last_loss 14.390→19.916, both steps_actual=22), confirming the lever was genuinely exercised. This mirrors the same lever's confirmed regression in the parked `continuous-openui-local` lineage's c5 frozen replay (structure 0.2308→0.1725) — a second, independent confirmation that `component_plan_loss_weight` regresses structural quality at this recipe/scale. The control arm's own latency_ms_p50 this cycle (12.58s, vs ~1.3-1.6s typical) is flagged as sandbox host-contention noise, not used as classification evidence. Gates fail closed on `insufficient_n` (3<20) and quality thresholds. **Non-positive (confirmed quality regression, not merely null); no repair_harness signal; never promote, sync, or ship** ([results](design/continuous-openui-local-2-20260802-c2-results.md)) |
| Autotrain continuous-openui-local-2 c1 grammar-bounds screen (fresh lineage, null) | `c20260802-continuous-openui-local--c94ddb78-c1-{control,bounds}` | CPU scratch TwoTower, `wf_smoke_v2`, lexer output, 20 steps (recorded 21), batch 2; both arms 1,608,962 trainable params | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-local--c94ddb78-c1/runs/` (local, explicit no-sync) | First cycle of a **new loop lineage** `continuous-openui-local-2` (`predecessor_campaign_id=null`), started because the prior `continuous-openui-local` lineage is correctly self-gated by its own unacknowledged `repair_harness` action (cycle 12, `harness_family=model_build`, decode-compiler performance on `test_data_scaleup_v1` n=12 — tracked separately in the open PR #1307; out of scope here). This cycle confirms the c12 harness fix (`_feasible_eval_version`) works on a genuinely fresh cycle: `eval_version=e938_role_safe_all_targets_v2` (smoke, n=3) was picked automatically and **both arms completed with zero decode timeouts**. Driver's thrash rotation independently picked `bounds` (`grammar_completion_bounds`) from the untested `_SCREENING_ARM_BANK`. Every decode-facing smoke quality metric is bit-identical between arms (structural_similarity 0.0575=0.0575, binder F1 0.6333=0.6333); only latency differs (p50 +3.98ms, p95 +134.21ms, candidate slower) with zero accompanying quality change. Gates fail closed on `insufficient_n` (3<20) and quality thresholds. **Non-positive; no repair_harness signal this cycle; never promote, sync, or ship** ([results](design/continuous-openui-local-2-20260802-c1-results.md)) |
| Autotrain continuous-openui-local c9 binder-topology screen at 2x steps (null) | `c20260802-continuous-openui-local-8c0b60dd-c9-{control,binder-topology}` | CPU scratch TwoTower, `wf_smoke_v2`, lexer output, **40 steps (escalated 2x from the prior cycles' 20-22, per this cycle's diagnostic scale-investigation step)**, seed 100009; both arms 2,137,346 trainable params | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-local-8c0b60dd-c9/runs/` (local, explicit no-sync) | Before screening, checked for a larger published eval snapshot (none exists — every snapshot under `src/slm_training/resources/data/eval/` shares smoke=3/held_out=5/adversarial=4/ood=4; `e938_role_safe_all_targets_v2` already has the largest `rico_held`) and confirmed steps could safely double within the wall cap (cycle 8's arms used only ~19s of a ~70s per-arm harness budget). Driver's hypothesizer selected `binder-topology` (`binder_topology_loss_weight=0.25`, `structural_aux_head_profile=binder-topology`), its rank-1 recommendation carried from cycle 8. Both trains completed cleanly at `steps=40` (`stopped_on=steps`, not a timeout; whole campaign ~31s wall) with distinct recipe (`binder_topology_loss_weight` 0.0 vs 0.25; `last_loss` 6.802424→6.833416, proving the escalated run genuinely trained differently). This was a screening-role cycle (`smoke` suite only, n=3; `held_out`/`adversarial`/`ood`/`rico_held` `missing_suite`). Despite the measurable loss divergence, every decode-facing quality metric on `smoke` is still bit-identical between arms (structural_similarity 0.51=0.51); only latency differs (+385.35ms, candidate slower). Primary metric delta is exactly 0.0 — a fourth consecutive bit-identical null this session and the **first at 2x training steps**, directly answering that escalating `--steps` (within the wall cap) does not reveal a signal these aux-loss levers produce on this size eval suite; the bottleneck is eval `n`, not training duration. Gates fail closed on `insufficient_n` (3<20) and quality thresholds. **Non-positive; binder-topology exhausted at this recipe, never promote, sync, or ship** ([results](design/continuous-openui-local-20260802-c9-results.md)) |
| Autotrain continuous-openui-local c8 component-inventory screen (null) | `c20260802-continuous-openui-local-8c0b60dd-c8-{control,component-inventory}` | CPU scratch TwoTower, `wf_smoke_v2`, lexer output, 20 steps (recorded 22), seed 100008; both arms 1,682,363 trainable params | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-local-8c0b60dd-c8/runs/` (local, explicit no-sync) | Driver's hypothesizer finally selected `component-inventory` (`component_inventory_loss_weight`, `structural_aux_head_profile=component-inventory`), its own rank-1 recommendation from cycles 6-7 that had twice been recommended but not selected. Both trains completed with distinct recipe (`component_inventory_loss_weight` 0.0 vs 1.0; `last_loss` 19.9561→21.1865) and a full `--ship-gates` scoreboard for each arm across `smoke` (n=3) and `held_out` (n=5) (`measurement_complete=true`). Every decode-facing quality metric is bit-identical between arms on both suites; only latency differs slightly (smoke +56.65ms, held_out +3.14ms, candidate slower). Primary metric (`held_out.structural_similarity`) delta is exactly 0.0 — a third bit-identical null this session (after c6 component-edge and c7 component-plan re-screen). Gates fail closed on `insufficient_n` (3<20, 5<20) and quality thresholds on both suites. **Non-positive; component-inventory exhausted at this recipe, never promote, sync, or ship** ([results](design/continuous-openui-local-20260802-c8-results.md)) |
| Autotrain continuous-openui-local c7 component-plan re-screen (null) | `c20260802-continuous-openui-local-8c0b60dd-c7-{control,component-plan}` | CPU scratch TwoTower, `wf_smoke_v2`, lexer output, 20 steps, seed 100007; both arms 1,755,764 trainable params | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-local-8c0b60dd-c7/runs/` (local, explicit no-sync) | Driver's hypothesizer re-selected `component-plan` (cycle 5's rejected lever) but as a fresh from-scratch seed-100007 pair, not a replay of c5's lineage. Both trains completed with distinct recipe (`component_plan_loss_weight` 0.0 vs 1.0; `last_loss` 15.1286→18.1728) and a full `--ship-gates` scoreboard for each arm (`measurement_complete=true`). Every decode-facing smoke metric is bit-identical between arms; only latency differs (1602.13→1383.81ms, candidate faster this time). Primary delta is exactly 0.0 — a genuine null at this seed, distinct from c5's confirmed regression (different checkpoint lineage); does not overturn c5's rejection. Gates fail closed on `insufficient_n` (3<20) and quality thresholds. **Non-positive; component-plan re-exhausted at this recipe, never promote, sync, or ship** ([results](design/continuous-openui-local-20260802-c7-results.md)) |
| Autotrain continuous-openui-local c6 component-edge screen (null) | `c20260802-continuous-openui-local-8c0b60dd-c6-{control,component-edge}` | CPU scratch TwoTower, `wf_smoke_v2`, lexer output, 20 steps, seed 100006; both arms 1,766,987 trainable params | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-local-8c0b60dd-c6/runs/` (local, explicit no-sync) | Both trains completed with distinct `config_sha256` (`component_edge_loss_weight` 0.0 vs 1.0) and a full `--ship-gates` scoreboard for each arm (`measurement_complete=true`). Every decode-facing smoke metric (parse_rate, meaningful_program_rate, structure, binder F1, component_type_recall, ast/canonical_beq_rate, placeholder_fidelity, reward_score) is bit-identical between arms; only latency differs (1451.38→1825.79ms, candidate slower). Primary delta is exactly 0.0 — a genuine null (not a regression), suggesting this fixture scale is under-powered to detect the lever's effect. Gates fail closed on `insufficient_n` (3<20) and quality thresholds. **Non-positive; component-edge exhausted at this recipe, never promote, sync, or ship** ([results](design/continuous-openui-local-20260802-c6-results.md)) |
| Autotrain continuous-openui-local c5 component-plan frozen replay (rejected) | `c20260802-continuous-openui-local-8c0b60dd-c5-{control,component-plan}` | CPU scratch TwoTower, `wf_smoke_v2`, lexer output, 20 steps; evaluation-only replay reusing c3's completed checkpoints (1,755,764 trainable params) | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-local-8c0b60dd-c5/runs/` (local, explicit no-sync) | Full `--ship-gates` scoreboard now completes for both arms (`measurement_complete=true`) after fixing a frozen-replay harness bug (`_apply_frozen_replay` truncated the hyphenated `component-plan` slug; `harness.autoresearch.experiment_campaign` v59→v60). Confirmed regression: structure 0.2308→0.1725, binder F1 0.7333→0.6333, meaningful 0.3333→0.0; gates fail closed on `insufficient_n` (3<20) and quality thresholds. **Non-positive; component-plan rejected for this recipe (confirmed quality regression, not merely exhausted); never promote, sync, or ship** ([results](design/continuous-openui-local-20260802-c5-results.md)) |
| Autotrain continuous-openui-local c3 component-plan incomplete screen | `c20260802-continuous-openui-local-8c0b60dd-c3-{control,component-plan}` | CPU scratch TwoTower, `wf_smoke_v2`, lexer output, 20 steps, seed 100003; both arms 1,755,764 trainable params | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-local-8c0b60dd-c3/runs/` (local, explicit no-sync) | Both trains completed; partial smoke n=3 diagnostic shows a regression (structure 0.2308→0.1725, binder F1 0.7333→0.6333, meaningful 0.3333→0.0) but full `--ship-gates` scoreboard was never written for either arm (`missing_scoreboard`) — the ambient `NODE_OPTIONS="--import tsx"` blocker from cycles 1-2 recurred because this cycle's driver invocation itself still inherited it. Fixed locally by re-running with `NODE_OPTIONS` unset for the whole process tree. **Superseded by the c5 frozen replay above, which completes the measurement; never promote, sync, or ship** ([results](design/continuous-openui-local-20260802-c3-results.md)) |
| Autotrain continuous-openui-local c2 grammar-bounds frozen replay | `c20260802-continuous-openui-local-8c0b60dd-c2-{control,bounds}` | CPU scratch TwoTower, `wf_smoke_v2`, lexer output, 20 steps; identical frozen replay of c1 after fixing the local AgentV/`NODE_OPTIONS` blocker | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-local-8c0b60dd-c2/runs/` (local, explicit no-sync) | Full `--ship-gates` scoreboard now completes for both arms (`measurement_complete=true`). smoke n=3 quality ties confirmed (parse 1.0, meaningful 0.0, structure .0575, binder F1 .6333); bounds p50 41.1ms faster but meaningful_program_rate stays 0.0 on both arms. Gates fail closed on `insufficient_n` (3<20) and quality thresholds. **Non-positive, confirms c1; bounds lever exhausted for this recipe, never promote, sync, or ship** ([results](design/continuous-openui-local-20260802-c2-results.md)) |
| Autotrain continuous-openui-local c1 grammar-bounds screen | `c20260802-continuous-openui-local-8c0b60dd-c1-{control,bounds}` | CPU scratch TwoTower, `wf_smoke_v2`, lexer output, 20 steps (recorded 21), seed 100001, batch 2; both arms 1,608,962 trainable params | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-local-8c0b60dd-c1/runs/` (local, explicit no-sync) | Both trains completed; smoke n=3 quality ties on the partial single-suite diagnostic (parse 1.0, meaningful 0.0, structure .0575, binder F1 .6333). Bounds p50 is 75.6ms faster but `meaningful_program_rate=0.0` on both arms, so the latency delta alone is not positive. Full `--ship-gates` scoreboard was never written for either arm (`missing_scoreboard`) because this sandbox's ambient `NODE_OPTIONS="--import tsx"` blocked every `node` subprocess, including the AgentV publish step; fixed locally via `env -u NODE_OPTIONS npm ci` (untracked `node_modules/`, no repo code change). **Superseded by the c2 frozen replay above, which completes the measurement and confirms non-positive; never promote, sync, or ship** ([results](design/continuous-openui-local-20260802-c1-results.md)) |
