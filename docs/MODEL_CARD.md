...
| E619 `slot_contract_in_context` scoring-gap replay | `e619-slot-contract-in-context-scratch80-20260720` | CPU scratch TwoTower train check | `outputs/runs/e619-slot-contract-in-context-scratch80-20260720/checkpoints/last.pt` (local) | Fresh 80-step run on E530 (244 records), replaying E616/E617's exact recipe; 1,643,522 trainable params, loss 26.5243, SHA `119dd41a…8898a854` (byte-identical to E616/E617). Matched OOD `n=4` E617-recipe control vs the same recipe plus `--slot-contract-in-context` treatment moves `required_inventory_coverage` from `UNKNOWN` to a real, correct `FAIL` on `ood_gallery_01` (coverage 0.75→1.0); every headline quality metric is unchanged and strict v2 stays 0.0→0.0 in both arms; **not promotable or ship** ([results](design/iter-e619-slot-contract-in-context-20260720.json)) |
