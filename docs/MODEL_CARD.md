...
| --- | --- | --- | --- | --- |
| Autotrain `continuous-openui-local` c1 bounds screen | `c20260802-continuous-openui-local-8c0b60dd-c1-{control,bounds}` | `fixture_or_scratch`: CPU scratch TwoTower, `wf_smoke_v2`, 21 steps, seed 100001, batch 2; both 1,608,962 params | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-local-8c0b60dd-c1/runs/` (local, explicit no-sync) | Exact smoke n=3 quality tie: parse 1.0, meaning 0, structure .0575, binder F1 .6333; bounds p50 1553.54 vs control 1594.76 ms. Both arms' `eval.json` scoreboard wrote, but the AgentV bundle publish step crashed on a missing `@agentv/core` install (`npm ci` had never run in this sandbox) — measurement_incomplete, not a quality result. `npm ci` was run after this cycle; frozen replay queued next. **Inconclusive; never reuse, promote, sync, or ship** ([results](design/continuous-openui-20260802-c1-results.md)) |
...
| --- | --- | --- | --- | --- |
| 2026-08-02 | `c20260802-continuous-openui-local-8c0b60dd-c1-{control,bounds}` | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-local-8c0b60dd-c1/runs/` (local, explicit no-sync) | Exact smoke n=3 tie: parse 1.0, meaning 0, structure .0575, binder F1 .6333; bounds p50 1553.54 vs control 1594.76 ms | `fixture_or_scratch`: CPU `wf_smoke_v2`, 21 steps, seed 100001, batch 2, matched 1,608,962 params. AgentV publish crashed (missing `@agentv/core`, fixed via `npm ci` post-cycle) so both arms are `measurement_incomplete`; frozen replay required before a new hypothesis. Neither checkpoint reusable/promotable/ship ([results](design/continuous-openui-20260802-c1-results.md)) |
