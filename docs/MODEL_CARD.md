...
| --- | --- | --- | --- | --- |
| Autotrain local-c3 component-plan screen | `c20260802-continuous-openui-local-8c0b60dd-c3-{control,component-plan}` | CPU scratch TwoTower, `wf_smoke_v2`, lexer output, 20 steps, batch default; control 1,608,962 vs component-plan 1,755,764 trainable params (component-plan head prebuilt) | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-local-8c0b60dd-c3/runs/` (local, explicit no-sync) | Smoke n=3: control parse 1.0 / meaningful .3333 / structure .2308 / binder F1 .7333; component-plan parse 1.0 / meaningful 0 / structure .1725 / binder F1 .6333 — component-plan **regresses** structural_similarity by -.0583 and fails a binder_reference_f1 non-regression check. AgentV complete; `insufficient_n` (3<20) plus quality gates fail. Clean code `74e9ca9583a99a2a7fe977414471fb47b1a6ffe5`, harness `v74`, meaning eval `2.13.0`, scoring `v22`. **Rejected; never reuse, promote, sync, or ship** ([results](design/continuous-openui-local-20260802-c3-results.md)) |
| Autotrain local-c2 frozen replay (bounds unblock) | `c20260802-continuous-openui-local-8c0b60dd-c2-{control,bounds}` | CPU scratch TwoTower, `wf_smoke_v2`, lexer output, 20 steps; both arms 1,608,962 trainable params (frozen replay of local-c1) | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-local-8c0b60dd-c2/runs/` (local, explicit no-sync) | Exact smoke n=3 quality ties: parse 1.0, meaningful 0, structure .0575, binder F1 .6333; bounds p50 1,243.80 vs control 1,232.53 ms. AgentV complete (unblocked by installing `@agentv/core` via `npm ci`); `insufficient_n` (3<20) plus quality gates fail; primary metric delta 0.0. **Rejected; never reuse, promote, sync, or ship** ([results](design/continuous-openui-local-20260802-c2-results.md)) |
| Autotrain local-c1 bounds screen (AgentV blocked) | `c20260802-continuous-openui-local-8c0b60dd-c1-{control,bounds}` | CPU scratch TwoTower, `wf_smoke_v2`, lexer output, 21 steps; both arms 1,608,962 trainable params | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-local-8c0b60dd-c1/runs/` (local, explicit no-sync) | Both trains completed and raw smoke metrics computed (parse 1.0, structure .0575 tied on both arms), but `evaluate_model --ship-gates` crashed on `_agentv_runtime` (`AgentV SDK is unavailable`) on a fresh checkout with no `npm ci` run yet — no scoreboard.json on either arm. Fixed in-session; identical arms replayed as local-c2. **Incomplete; never reuse, promote, sync, or ship; frozen replay required** ([results](design/continuous-openui-local-20260802-c1-results.md)) |
...
| --- | --- | --- | --- | --- |
| 2026-08-02 | `c20260802-continuous-openui-local-8c0b60dd-c3-{control,component-plan}` | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-local-8c0b60dd-c3/runs/` (local, explicit no-sync) | Smoke n=3 structural_similarity: control .2308, component-plan .1725 (delta -.0583); binder_reference_f1 regressed .7333→.6333; AgentV complete, gates fail | CPU `wf_smoke_v2`, 20 steps; control 1,608,962 vs component-plan 1,755,764 trainable params. Control SHA `2a97c06708202c1…`; component-plan SHA `d3e77a05d69626a…`. Clean code `74e9ca9583a99a2a7fe977414471fb47b1a6ffe5`, harness `v74`, meaning eval `2.13.0`, scoring `v22`. Quality regression; neither checkpoint reusable/promotable/ship ([results](design/continuous-openui-local-20260802-c3-results.md)) |
| 2026-08-02 | `c20260802-continuous-openui-local-8c0b60dd-c2-{control,bounds}` | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-local-8c0b60dd-c2/runs/` (local, explicit no-sync) | Exact smoke n=3 quality tie: parse 1.0, meaning 0, structure .0575; bounds p50 1,243.80 vs control 1,232.53 ms; AgentV complete, gates fail | CPU `wf_smoke_v2`, 20 steps; both arms 1,608,962 trainable params (frozen replay of local-c1 after installing `@agentv/core` via `npm ci`). Clean code `b51ba33e253b45484bbfc5d9deefa40f228bdd12`. Quality null, `insufficient_n`; neither checkpoint reusable/promotable/ship ([results](design/continuous-openui-local-20260802-c2-results.md)) |
| 2026-08-02 | `c20260802-continuous-openui-local-8c0b60dd-c1-{control,bounds}` | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-local-8c0b60dd-c1/runs/` (local, explicit no-sync) | Both trains completed, raw smoke metrics tied (structure .0575), but `evaluate_model --ship-gates` crashed on missing AgentV SDK — no scoreboard on either arm | CPU `wf_smoke_v2`, 21 steps; both arms 1,608,962 trainable params. Control SHA `c2ba05273c48aac…`; bounds SHA `b040e8f48b955e9…`. Fresh-checkout `npm ci` gap, fixed in-session. Incomplete; frozen replay required (see local-c2) ([results](design/continuous-openui-local-20260802-c1-results.md)) |
