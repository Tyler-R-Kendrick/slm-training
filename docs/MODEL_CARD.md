...
| --- | --- | --- | --- | --- |
| Autotrain p33lme c4 component-edge measurement-incomplete screen | `c20260802-continuous-openui-p33lme-489d3aa7-c4-{control,component-edge}` | CPU scratch TwoTower, `wf_smoke_v2`, lexer output, 21 steps, seed 100004, batch 2; both arms 1,766,987 trainable params | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-p33lme-489d3aa7-c4/runs/` (local, explicit no-sync) | Both arms trained to completion, but `evaluate_model --ship-gates` for this promotion cycle's `smoke,held_out` suite pair exceeded its stage wall-time budget before either arm wrote `scoreboard.json`. Partial smoke telemetry ties (structural_similarity 0.4167=0.4167); `held_out` never scored. **Inconclusive; measurement incomplete, exact frozen replay only (`retry_measurement` queued), never reuse, promote, sync, or ship** ([results](design/autotrain-cycle-p33lme-c4-component-edge-screen.md)) |
| Autotrain p33lme c3 component-plan aux-head screen | `c20260802-continuous-openui-p33lme-489d3aa7-c3-{control,component-plan}` | CPU scratch TwoTower, `wf_smoke_v2`, lexer output, 20 steps, seed 100003, batch 2; both arms 1,755,764 trainable params | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-p33lme-489d3aa7-c3/runs/` (local, explicit no-sync) | Smoke n=3: candidate regresses vs control on structural_similarity (.2308→.1725), meaningful_program_rate (.3333→0), and binder F1 (.7333→.6333), and is 6.53% slower (3057.79→3257.47 ms p50). AgentV complete (`execution_errors=0`); `--ship-gates` fail on fixture `n` and thresholds. **Rejected; never reuse, promote, sync, or ship** ([results](design/autotrain-cycle-p33lme-c3-component-plan-screen.md)) |
| Autotrain p33lme c1 AgentV NODE_OPTIONS repair screen | `c20260802-continuous-openui-p33lme-489d3aa7-c1-{control,bounds}` | CPU scratch TwoTower, `wf_smoke_v2`, lexer output, 20 steps, batch 2; both arms 1,608,962 trainable params | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-p33lme-489d3aa7-c1/runs/` (local, explicit no-sync) | Both arms trained and decoded smoke n=3 to completion, but `evaluate_model --ship-gates` raised before writing `scoreboard.json` (AgentV SDK node child inherited a broken `NODE_OPTIONS`, exit 9) — measurement incomplete, no comparison metrics available. Fixed in `a82eea405548cea5fcc486c3332823e2e3b008cf` (`evals.agentv` v6→v7); smoke `structural_similarity` tie (control 0.0575 = candidate 0.0575) was recorded from partial telemetry only. **Inconclusive; exact frozen replay only (`retry_measurement` queued), never promote, sync, or ship** ([results](design/autotrain-cycle-p33lme-c1-agentv-node-options-repair.md)) |
