...
| --- | --- | --- | --- | --- |
| 2026-08-01 | `c20260801-continuous-openui-bqw0tb-f852ac38-c5-{control,batch1}` | `outputs/autoresearch/continuous-loop-20260801-continuous-openui-bqw0tb-f852ac38-c5/runs/*/checkpoints/last.pt` (local, explicit no-sync) | control completed (fixture `insufficient_n` gate fail, expected); `batch1` (batch_size=1 lever) timed out mid-decode under `MAX_RUN_MINUTES`, no metric produced | Soft wall-timeout on the treatment arm, never evidence per the iron law; not a harness defect. Frozen replay required (see [results](design/continuous-openui-bqw0tb-c5-results.md)); not promoted, reused, or shipped |
| 2026-08-01 | `c20260801-continuous-openui-bqw0tb-f852ac38-c4-{control,both}` | `outputs/autoresearch/continuous-loop-20260801-continuous-openui-bqw0tb-f852ac38-c4/runs/*/checkpoints/last.pt` (local, explicit no-sync) | CPU scratch TwoTower, `wf_smoke_v2`, 21 steps, batch 2; smoke `n=3`: parse 1.0, `binder_reference_f1` 0.7222 identical on both arms (0.0 delta), `meaningful_program_rate` 0.0, latency_ms_p50 9624.39 (control) / 10286.11 (bounds+canvas) | Frozen replay of c3 after the AgentV SDK fix (see [results](design/continuous-openui-bqw0tb-c4-results.md)); combined `grammar_completion_bounds`+`compact_active_canvas` gave zero primary-metric delta and higher latency; ship gates fail on fixture n; not promoted, reused, or shipped |
| 2026-08-01 | `c20260801-continuous-openui-bqw0tb-f852ac38-c3-{control,both}` | `outputs/autoresearch/continuous-loop-20260801-continuous-openui-bqw0tb-f852ac38-c3/runs/*/checkpoints/last.pt` (local, explicit no-sync) | CPU scratch TwoTower, `wf_smoke_v2`, 21 steps, batch 2; train completed (`binder_reference_f1=0.7222` identical on both arms) but eval publication crashed on a missing/misconfigured AgentV SDK (`node_modules/@agentv/core` absent + sandbox `NODE_OPTIONS` rejected by Node) | Infrastructure-only provenance for this pair; frozen replay required after `npm ci` + `NODE_OPTIONS` sanitization (see [results](design/continuous-openui-bqw0tb-c3-results.md)); never reuse, promote, or ship |
| 2026-08-01 | `c20260801-continuous-openui-bqw0tb-f852ac38-c2-{control,bounds}` | `outputs/autoresearch/continuous-loop-20260801-continuous-openui-bqw0tb-f852ac38-c2/runs/*/checkpoints/last.pt` (local, explicit no-sync) | CPU scratch TwoTower, `wf_smoke_v2`, 21 steps, batch 2; smoke `n=3`: parse 1.0, `binder_reference_f1` 0.6333 identical on both arms (0.0 delta), `meaningful_program_rate` 0.0, latency_ms_p50 4838.87 (control) / 5872.59 (bounds) | Screening replay after the c1 torch-missing infra fail (see [results](design/continuous-openui-bqw0tb-c1-results.md), [c2 results](design/continuous-openui-bqw0tb-c2-results.md)); `grammar_completion_bounds` produced zero primary-metric delta and higher latency; honest ship gates fail on fixture n; not promoted, reused, or shipped |
