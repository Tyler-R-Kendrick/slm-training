...
| --- | --- | --- | --- | --- |
| SLM-313 AbstractPlan local checkpoint | `slm313_local_plan_1k_v1` | CPU scratch Choice TwoTower with predicted-plan head and learned connector conditioning; 8 canonical direct-natural rows | `outputs/runs/slm313_local_plan_1k_v1/checkpoints/last.pt` (local, explicit no-sync) | 9 steps / 1,006 target tokens / 18.47s, SHA `15bfaf30…033ac6`; locked plan-control matrix has not run, so there are no semantic metrics and it is **not promotable, reusable, or ship** ([SLM-313 evidence](design/abstract-plan-functional-evidence.md)) |
