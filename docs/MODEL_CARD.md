...
| --- | --- | --- | --- | --- |
| SLM-230 bounded recurrence diagnostic | `slm230_bounded_recursive_r4_r2` | CPU scratch symbol-only shared-recursive R=4 diagnostic | `outputs/runs/slm230_bounded_recursive_r4_r2/checkpoints/last.pt` (local) | 4 steps / 1.70s, SHA `1604b2cb…8b28a`; held-out depth 1→4 CE improves on both records, but accuracy/parse/structure/reward stay 0.0 and no anytime exit qualifies (`stagnant`). SLM-231 then measures an exact one-token state product with top singular value `4.7243` / max FTLE `0.3882` (`expansive_unstable`) — **explicit no-sync, rejected, not ship** ([observability](design/iter-slm230-recurrence-observability-20260724.md), [dynamics](design/iter-slm231-recurrence-dynamics-20260724.md)) |
