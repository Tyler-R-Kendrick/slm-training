...
| --- | --- | --- | --- | --- |
| Autotrain `continuous-openui-jkk2fb` c2 component-plan screen | `c20260802-continuous-openui-jkk2fb-9f2e5830-c2-{control,component-plan}` | CPU scratch TwoTower, `wf_smoke_v2`, control 22 / candidate 20 steps, both arms 1,755,764 trainable params (component-plan head prebuilt on candidate) | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-jkk2fb-9f2e5830-c2/runs/` (local, explicit no-sync) | Control trained to completion (`stopped_on=steps`, last_loss 14.39) but hit the per-arm wall_timeout before evaluation ran — no control suite metrics. Candidate smoke n=3: parse 1.0, meaningful 0.0, structure .0964, binder F1 0.0, p50 2088.0 ms; ship-gate publish incomplete (AgentV SDK unavailable). **Incomplete measurement; never reuse, promote, sync, or ship** ([results](continuous-openui-jkk2fb-c2-results.md)) |
| Autotrain `continuous-openui-jkk2fb` c1 bounds screen | `c20260802-continuous-openui-jkk2fb-9f2e5830-c1-{control,bounds}` | CPU scratch TwoTower, `wf_smoke_v2`, 20 steps, both arms 1,608,962 trainable params (`grammar_completion_bounds` off vs on) | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-jkk2fb-9f2e5830-c1/runs/` (local, explicit no-sync) | Exact smoke n=3 quality ties: parse 1.0, meaningful 0.0, structure .0575, binder F1 .6333. Treatment p50 2380.30 vs control 2469.22 ms is a latency-only delta with zero quality meaning — not a positive tradeoff. Ship-gate publish step failed (`AgentV SDK unavailable`, `npm ci` not run this session); smoke metrics captured pre-publish. **Rejected; never reuse, promote, sync, or ship** ([results](continuous-openui-jkk2fb-c1-results.md)) |
...
| --- | --- | --- | --- | --- |
| 2026-08-02 | `c20260802-continuous-openui-jkk2fb-9f2e5830-c2-{control,component-plan}` | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-jkk2fb-9f2e5830-c2/runs/` (local, explicit no-sync) | Control trained (22 steps, last_loss 14.39) but wall-timed-out before eval; candidate smoke n=3 parse 1.0, meaningful 0.0, structure .0964, binder F1 0.0, p50 2088.0 ms; ship-gate publish incomplete | CPU `wf_smoke_v2`, matched 1,755,764-param arms (component-plan head). `measurement_complete: false`, `primary_metric_unavailable` — incomplete, not positive; neither checkpoint reusable/promotable/ship ([results](continuous-openui-jkk2fb-c2-results.md)) |
| 2026-08-02 | `c20260802-continuous-openui-jkk2fb-9f2e5830-c1-{control,bounds}` | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-jkk2fb-9f2e5830-c1/runs/` (local, explicit no-sync) | Exact smoke n=3 quality ties: parse 1.0, meaningful 0.0, structure .0575, binder F1 .6333; treatment p50 2380.30 vs control 2469.22 ms; ship-gate publish incomplete (AgentV SDK unavailable) | CPU `wf_smoke_v2`, 20 steps, matched 1,608,962-param arms (`grammar_completion_bounds` off/on). Non-positive per `_classify_metric_tradeoff` (latency-only delta, zero quality movement); neither checkpoint reusable/promotable/ship ([results](continuous-openui-jkk2fb-c1-results.md)) |
