...
| E620 `slot_contract_in_context` live control (corrects E618/E619) | `e619-slot-contract-in-context-scratch80-20260720` | CPU scratch TwoTower train check | `outputs/runs/e619-slot-contract-in-context-scratch80-20260720/checkpoints/last.pt` (local) | Fresh 80-step run on E530 (244 records), replaying E616/E617/E619's exact recipe; 1,643,522 trainable params, loss 26.5243, SHA `119dd41a…8898a854` (byte-identical to E616/E617/E619). Matched OOD `n=4` E617-recipe control (flag genuinely unset, live) vs the same recipe plus `--slot-contract-in-context` treatment moves `required_inventory_coverage` from `UNKNOWN` to a real, correct `FAIL` on `ood_gallery_01` only (coverage 0.75→1.0) — correcting E618's and the concurrent E619's "every/all 4 records" framing; every headline quality metric is unchanged and strict v2 stays 0.0→0.0 in both arms; **not promotable or ship** ([results](design/iter-e620-slot-contract-in-context-control-20260720.json)) |
