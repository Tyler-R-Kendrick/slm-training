...
| 2026-07-17 | G5 meta-trace fixture corpus (`g5_fixture_20260717`, no checkpoint) | `outputs/experiments/meta_traces/` (local); evidence `docs/design/iter-g5-meta-traces-20260717.md`; schema in `docs/design/telemetry.md` | 76 typed records harvested from four real run dirs (E280/E281/E282/X22-reeval) with trace-id joins, gate verdicts, and predictions; deterministic replay proof passes (tree-edit decode reproduces bit-exact from checkpoint sha) | Schema+retention only, per G5 scope; per-step trajectories populated only via collect_trajectories; MaskGIT replay is from-spec not bit-exact; grammar matrix emits no trace bundles yet (noted follow-up). Wiring-only; nothing promoted Harvest tooling (`meta_trace.py`) was consolidated into the canonical `harnesses/distill/trace_store.py` G5 owner during the #275 merge and the parallel module removed; the ledger row is retained as an experiment record. |
