...
| --- | --- | --- | --- | --- |
| Autotrain c1737 component-plan screen (measurement incomplete) | `c20260802-continuous-openui-202607-98199209-c2-{control,component-plan}` | CPU scratch TwoTower, `wf_smoke_v2`, lexer output, 22 steps, seed 100001, batch 2; control decode interrupted by wall cap (2/3 smoke records), treatment 1,755,764 trainable params with `component_plan_loss_weight=1.0` | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-202607-98199209-c2/runs/` (local, explicit no-sync) | Control smoke decode did not finish inside `MAX_RUN_MINUTES`, so no primary-metric comparison exists. Treatment completed alone: parse 1.0, binder F1 0, meaningful 0, structure .0964 — honestly fails every quality gate on fixture n=3. Soft wall-timeout, not a rejection of the lever. **No valid comparison; never reuse, promote, sync, or ship** ([results](design/autotrain-cycle-1737-component-plan-timeout-screen.md)) |
| Autotrain c1736 grammar-completion-bounds screen | `c20260802-continuous-openui-202607-98199209-c1-{control,bounds}` | CPU scratch TwoTower, `wf_smoke_v2`, lexer output, 21 steps, seed 100001, batch 2; evaluation `honest_slot_contract=True`; matched-size arms differing only in `grammar_completion_bounds` | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-202607-98199209-c1/runs/` (local, explicit no-sync) | Exact smoke n=3 quality ties: parse 1.0, meaningful 0, structure .0575, binder F1 .6333, AST node/edge F1 0/0. Treatment p50 2,394.46 vs control 2,429.84 ms (1.46% faster) but `meaningful_program_rate=0` is below the 1/3 floor required to credit a latency-only win, so the run is non-positive. AgentV publish initially failed (`node_modules` absent in this checkout; fixed with `npm ci`); both arms then completed honest gates and **fail**. **Rejected; never reuse, promote, sync, or ship** ([results](design/autotrain-cycle-1736-grammar-completion-bounds-screen.md)) |
...
| --- | --- | --- | --- | --- |
| 2026-08-02 | `c20260802-continuous-openui-202607-98199209-c2-{control,component-plan}` | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-202607-98199209-c2/runs/` (local, explicit no-sync) | Control smoke decode wall-timed out (2/3 records); treatment completed alone: parse 1.0, binder F1 0, meaningful 0, structure .0964, gates fail | CPU `wf_smoke_v2`, 22 steps, seed 100001, batch 2. Control SHA `417b3ce8…423c274cc`; treatment SHA `68f1ce95…6bf903bdf`. Clean code `a4ce8e3a3ecbd1ab7a8b0b94887b603fc5c5f346`, harness `v73`, meaning eval `2.13.0`, scoring `v22`, gates `openui_ship_gates_v6`. No valid control comparison this cycle (soft wall-timeout); neither checkpoint reusable/promotable/ship ([results](design/autotrain-cycle-1737-component-plan-timeout-screen.md)) |
| 2026-08-02 | `c20260802-continuous-openui-202607-98199209-c1-{control,bounds}` | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-202607-98199209-c1/runs/` (local, explicit no-sync) | Exact smoke n=3 quality ties: parse 1.0, meaningful 0, structure .0575, binder F1 .6333; treatment p50 2,394.46 vs 2,429.84 ms; AgentV complete after `npm ci`, gates fail | CPU `wf_smoke_v2`, 21 steps, seed 100001, batch 2, evaluation `honest_slot_contract=True`. Control SHA `c2ba0527…f6f7eda2`; treatment SHA `b040e8f4…31f8e155`. Clean code `62f315565318479b83e6352e1b0d75ab66011b9b`, harness `v73`, meaning eval `2.13.0`, scoring `v22`, gates `openui_ship_gates_v6`. 1.46% latency gain disqualified by `meaningful_program_rate=0` floor (needs ≥1/3); neither checkpoint reusable/promotable/ship ([results](design/autotrain-cycle-1736-grammar-completion-bounds-screen.md)) |
