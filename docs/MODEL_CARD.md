...
| E615 object-frame schema-role slot bias | `e615_training_loop_twotower_scratch_20260720` | CPU scratch TwoTower train check | `outputs/runs/e615_training_loop_twotower_scratch_20260720/checkpoints/last.pt` (local) | Fresh 8-step run on E530 (244 records), 1,643,522 trainable params, loss 37.9045, SHA `c083f8d1…3eef0c508`. Matched OOD `n=4` control (`schema_role_slot_decode_weight=0`) vs treatment (`=8.0`) eval pair run against this checkpoint — byte-identical, 0/4 parseable predictions on both, checkpoint too undertrained for the object-frame `src`/`alt`/`details` fix to be observed end-to-end; **not promotable or ship** ([results](design/iter-e615-object-frame-schema-role-slot-20260720.json)) |
...
| E622 training-time decode-gate check (40-step) | `e622-training-time-decode-gate-scratch40-20260720` | CPU scratch TwoTower train check | `outputs/runs/e622-training-time-decode-gate-scratch40-20260720/checkpoints/last.pt` (local) | Fresh 40-step run on E530 (244 records), loss 15.3967, SHA `6b7aaf2b…9037bf00`. First checkpoint in the E610-E621 lineage where `self._slot_contracts` (E617's gate) is populated by training's own periodic eval, but `train_model.py`'s CLI exposed zero of the weighted decode levers behind it (all `0.0`) — `ood` `n=4`: `meaningful_program_rate` 0.0, `reward_score` 0.0, `ship_score` 0.0347; **not promotable or ship** ([results](design/iter-e622-training-time-decode-gate-scratch40-20260720.json)) |
| E623 decode-weight CLI-gap control/treatment (40-step) | `e623-control-scratch40-20260720` / `e623-treatment-scratch40-20260720` | CPU scratch TwoTower paired train check | `outputs/runs/e623-{control,treatment}-scratch40-20260720/checkpoints/last.pt` (local) | Control byte-reproduces E622's checkpoint (SHA `6b7aaf2b…9037bf00`, loss 15.3967); treatment (SHA `3dec6e98…2911844c`) is the same trained weights with the newly-wired `semantic_plan_*`/`schema_*`/`schema_role_slot_decode_weight` CLI flags (E617's values) turned on for the first time inside a real `slm sft train` periodic eval. `ood` `n=4` delta: `meaningful_program_rate` 0.0→0.75, `reward_score` 0.0→0.6748, `ship_score` 0.0347→0.6572, single-seed; **not promotable or ship** ([results](design/iter-e623-decode-weight-cli-gap-closed-scratch40-20260720.json)) |
| E624 multi-seed replication (40-step, seed 1/2) | `e624-{control,treatment}-scratch40-seed{1,2}-20260720` | CPU scratch TwoTower paired train check (x2 seeds) | `outputs/runs/e624-{control,treatment}-scratch40-seed{1,2}-20260720/checkpoints/last.pt` (local) | Four fresh checkpoints replicating E623's control/treatment pair at `seed=1` (SHAs `2e51fe1c…8d478792` / `f821027c…d3f844fe`) and `seed=2` (SHAs `e6f3a891…6b68f69f` / `3239025b…dc63cce7`). Mixed result: `seed=2` reproduces E623's large positive delta (`reward_score` +0.825, `ship_score` +0.529); `seed=1`'s control decode is already strong unbiased (`reward_score=0.8475`), so treatment is flat-to-negative there (`reward_score` −0.0255, `ship_score` −0.0291) — narrows E623's single-seed claim to conditional, not unconditional; **not promotable or ship** ([results](design/iter-e624-multiseed-replication-decode-weight-20260720.json)) |
| E634 required-slot coverage replay (800-step) [renumbered from main's E620; nucd4x already used E620 for different work] | `e620-required-slot-coverage-scratch800-20260720` | CPU scratch TwoTower duration diagnostic | `outputs/runs/e620-required-slot-coverage-scratch800-20260720/checkpoints/last.pt` (local) | 800 steps on E530 in 80.96s, 1,643,522 trainable params, loss 4.0680, SHA `3ce5c9ef…363ecc5f`. Matched OOD `n=4` treatment reaches fidelity 0.5500 / structure 0.4886 / reward 0.8140 but strict-v2 remains 0.0 and AgentV 0/1; lower train loss regresses E619 fidelity/structure, so **rejected, not promotable or ship** ([results](design/iter-e634-required-slot-coverage-scratch800-20260720.md)) |
