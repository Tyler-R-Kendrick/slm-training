...
| 2026-07-17 | G5 meta-trace fixture corpus (`g5_fixture_20260717`, no checkpoint) | `outputs/experiments/meta_traces/` (local); evidence `docs/design/iter-g5-meta-traces-20260717.md`; schema in `docs/design/telemetry.md` | 76 typed records harvested from four real run dirs (E277/E278/E279/X22-reeval) with trace-id joins, gate verdicts, and predictions; deterministic replay proof passes (tree-edit decode reproduces bit-exact from checkpoint sha) | Schema+retention only, per G5 scope; per-step trajectories populated only via collect_trajectories; MaskGIT replay is from-spec not bit-exact; grammar matrix emits no trace bundles yet (noted follow-up). Wiring-only; nothing promoted |
