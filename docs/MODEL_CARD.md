...
| --- | --- | --- | --- | --- |
| Autotrain continuous-local c3 component-edge screen | `c20260802-continuous-openui-local-8c0b60dd-c3-{control,component-edge}` | CPU scratch TwoTower, `wf_smoke_v2`, lexer output, 20 steps; both arms 1,766,987 trainable params | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-local-8c0b60dd-c3/runs/` (local, explicit no-sync) | Both AgentV smoke n=3 arms completed with **identical** parse 1.0, meaningful .3333, structure .2308, binder F1 .7333; component-edge p50 3,818.46 vs control 4,267.38 ms (-10.5% wall, +11.76% `mpr_per_ms`), above the 5% efficiency floor with quality held. SDLC Phase A: **POSITIVE** (efficiency screening signal). Honest gates still fail (insufficient n=3). **Screening win, not confirmed/promoted/ship** ([results](design/continuous-openui-20260802-c3-results.md)) |
| Autotrain continuous-local c2 component-plan screen | `c20260802-continuous-openui-local-8c0b60dd-c2-{control,component-plan}` | CPU scratch TwoTower, `wf_smoke_v2`, lexer output, 20 steps; both arms 1,755,764 trainable params | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-local-8c0b60dd-c2/runs/` (local, explicit no-sync) | Both AgentV smoke n=3 arms completed; honest gates fail (insufficient n). Component-plan structural_similarity 0.0964 vs control 0.3267 (-70.5% relative) is a real quality regression. **Rejected; never reuse, promote, sync, or ship** ([results](design/continuous-openui-20260802-c2-results.md)) |
| Autotrain continuous-local c1 infra screen | `c20260802-continuous-openui-local-8c0b60dd-c1-{control,bounds}` | CPU scratch TwoTower, `wf_smoke_v2`, lexer output, 21 steps | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-local-8c0b60dd-c1/runs/` (local, explicit no-sync) | Both arms trained; `evaluate_model --ship-gates` crashed on both (AgentV SDK unavailable, fixed in-cycle via `npm ci`), so the reported `structural_similarity` 0.0575/0.0575 is a crash fallback, not a measured tie. **Infrastructure-only; never reuse, promote, sync, or ship** ([results](design/continuous-openui-20260802-c1-results.md)) |
...
| --- | --- | --- | --- | --- |
| 2026-08-02 | `c20260802-continuous-openui-local-8c0b60dd-c3-{control,component-edge}` | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-local-8c0b60dd-c3/runs/` (local, explicit no-sync) | Both smoke n=3: parse 1.0, meaningful .3333, structure .2308, binder F1 .7333 (identical); control p50 4,267.38ms vs component-edge p50 3,818.46ms (-10.5%, +11.76% mpr_per_ms); gates fail (insufficient n) | Size-matched 1,766,987-param arms. Quality held exactly, latency win above 5% floor -- SDLC Phase A POSITIVE (screening signal); needs new-seed confirmation before promotion. Neither checkpoint reusable/promotable/ship yet ([results](design/continuous-openui-20260802-c3-results.md)) |
| 2026-08-02 | `c20260802-continuous-openui-local-8c0b60dd-c2-{control,component-plan}` | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-local-8c0b60dd-c2/runs/` (local, explicit no-sync) | Both smoke n=3: parse 1.0; control structural_similarity 0.3267 vs component-plan 0.0964 (-70.5%); control p50 12,662.9ms vs component-plan p50 1,406.4ms; gates fail (insufficient n) | Size-matched 1,755,764-param arms. Component-plan is a real quality regression on this seed/step count, not a null; rejected. Neither checkpoint reusable/promotable/ship ([results](design/continuous-openui-20260802-c2-results.md)) |
| 2026-08-02 | `c20260802-continuous-openui-local-8c0b60dd-c1-{control,bounds}` | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-local-8c0b60dd-c1/runs/` (local, explicit no-sync) | Both arms trained (21 steps, CPU); eval crashed on both, `structural_similarity` 0.0575/0.0575 is a crash fallback | AgentV SDK was unavailable in this fresh sandbox (`npm ci` never run); fixed in-cycle. SDLC Phase A correctly classified NON_POSITIVE on the null delta. Neither checkpoint reusable/promotable/ship ([results](design/continuous-openui-20260802-c1-results.md)) |
