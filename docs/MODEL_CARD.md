...
| --- | --- | --- | --- | --- |
| Autotrain mpjoo9-c2 component-plan wall-timeout | `c20260802-continuous-openui-mpjoo9-2b5d9d52-c2-{control,component-plan}` | CPU scratch TwoTower, `wf_smoke_v2`, 22 steps, batch 2; both arms 1,755,764 trainable params (component-plan head prebuilt) | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-mpjoo9-2b5d9d52-c2/runs/` (local, explicit no-sync) | Both arms trained to completion, but the control's `evaluate_model` call never ran (per-arm wall budget `max_wall_minutes=1.1667` exhausted) — no honest comparison possible. Candidate alone: parse 1.0, meaning 0, structure .0964, p50 2,300.04 ms; `--ship-gates` fails, `AgentV SDK is unavailable`. Control checkpoint SHA `417b3ce8…423c274cc`; candidate SHA `68f1ce95…6bf903bdf`. Clean code `a72a453e17bc3f8218ebc802d16cd3f4e9f18ee2`, harness `v28`. **Rejected; never reuse, promote, sync, or ship** ([results](design/autotrain-mpjoo9-c2-component-plan-timeout.md)) |
| Autotrain mpjoo9-c1 bounds screen | `c20260802-continuous-openui-mpjoo9-2b5d9d52-c1-{control,bounds}` | CPU scratch TwoTower, `wf_smoke_v2`, 21 steps, seed 100001, batch 2; both arms 1,608,962 trainable params | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-mpjoo9-2b5d9d52-c1/runs/` (local, explicit no-sync) | Exact smoke n=3 quality ties: parse 1.0, binder F1 .6333, meaning 0, structure .0575, AST node/edge F1 0/0. Candidate p50 2,328.11 vs control 2,275.05 ms is 2.33% **slower**, below any efficiency floor. `evaluate_model --ship-gates` fails on both arms with `AgentV SDK is unavailable` (Node.js/npm toolchain absent in this sandbox); base suite scoring completed. Clean code `20ae71ff43051edc79d2475a51848a0bd5adb7f0`, harness `v73`, meaning eval `2.13.0`, scoring `v22`. **Rejected; never reuse, promote, sync, or ship** ([results](design/autotrain-mpjoo9-c1-bounds-screen.md)) |
...
| --- | --- | --- | --- | --- |
| 2026-08-02 | `c20260802-continuous-openui-mpjoo9-2b5d9d52-c2-{control,component-plan}` | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-mpjoo9-2b5d9d52-c2/runs/` (local, explicit no-sync) | Control trained but evaluation never ran (per-arm wall budget exhausted); candidate alone parse 1.0, structure .0964, p50 2,300.04 ms; `--ship-gates` fails, AgentV unavailable | CPU `wf_smoke_v2`, 22 steps, batch 2, size-matched 1,755,764-param arms (component-plan head prebuilt). Control SHA `417b3ce8…423c274cc`; candidate SHA `68f1ce95…6bf903bdf`. Clean code `a72a453e17bc3f8218ebc802d16cd3f4e9f18ee2`, harness `v28`. No usable comparison; neither checkpoint reusable/promotable/ship ([results](design/autotrain-mpjoo9-c2-component-plan-timeout.md)) |
| 2026-08-02 | `c20260802-continuous-openui-mpjoo9-2b5d9d52-c1-{control,bounds}` | `outputs/autoresearch/continuous-loop-20260802-continuous-openui-mpjoo9-2b5d9d52-c1/runs/` (local, explicit no-sync) | Exact smoke n=3 quality tie: parse 1.0, binder F1 .6333, meaning 0, structure .0575; candidate p50 2,328.11 vs control 2,275.05 ms (2.33% slower); AgentV base scoring complete, `--ship-gates` publish fails (AgentV SDK unavailable, no npm/Node.js in sandbox) | CPU `wf_smoke_v2`, 21 steps, seed 100001, batch 2, matched 1,608,962-param arms. Control SHA `c2ba0527…6f7eda2`; candidate SHA `b040e8f4…31f8e155`. Clean code `20ae71ff43051edc79d2475a51848a0bd5adb7f0`, harness `v73`, meaning eval `2.13.0`, scoring `v22`, gates `openui_ship_gates_v6`. Primary metric null delta; neither checkpoint reusable/promotable/ship ([results](design/autotrain-mpjoo9-c1-bounds-screen.md)) |
