...
- `realize_surface_and_verify(solved_program, *, pack, realizer, opaque_bindings,
  ...
  1. requires a solved semantic-IR fingerprint and prior `solved`/`verified`
     status;
  2. extracts/classifies surface slots via `pack.surface_slot_extractor`;
...
- Added optional slots to `DslPack`:
  - `surface_slot_extractor`
  - `surface_assignment_validator`
  - `surface_applier`
  - `surface_oracle`
- Registered `_openui_surface_slot_extractor` for the `openui` pack:
...
- `ruff check src/slm_training/dsl/surface.py src/slm_training/dsl/pack.py tests/test_dsl/test_surface_realization.py` passes.
- `python -m compileall` passes.
- `pytest tests/test_dsl/test_surface_realization.py tests/test_dsl/test_choice_codec.py tests/test_dsl/test_opaque_regions.py tests/test_dsl/test_pack.py -q` → 68 passed.
- `python -m scripts.repo_policy` ok.
- `git diff --check` clean.
- `.githooks/check-changed` passes except for the pre-existing unrelated failure in
  `tests/test_data/test_verify.py::test_preview_runtime_and_behavior_seeded_failures`
  (missing `src/src/.../preview.js` build artifact).
...
- Only `INTERNAL_IDENTIFIER` and `OPAQUE_USER_VALUE` realization is implemented
  ...
  not yet wired.
- `surface_assignment_validator`, `surface_applier`, and `surface_oracle` are
  reserved pack slots for future per-language override; the default pipeline
  uses direct substitution, VSS2-04 splicing, `pack.canonicalize`, and
  `pack.oracle`.
