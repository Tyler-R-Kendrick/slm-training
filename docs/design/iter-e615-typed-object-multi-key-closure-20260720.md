...
(from the active object schema). E614's own fixture and unit
...
1. **Component-argument branch.** A synthetic `_ChoiceFrame` pushed directly
   ...
   derives `required_properties` from the synthetic schema exactly as the
...
2. **No-required regression.** The same synthetic-frame technique with an
   ...
   today) confirms required-property closure leaves those objects unaffected — a bare
   close stays legal.
...
by this iteration; no new runtime lever is introduced.
