# OpenFeature representation of autoresearch experiments
...
Related: [`openfeature-experiments.md`](openfeature-experiments.md) covers the
**product experiment** layer (`slm_training.features` runtime with
LaunchDarkly/PostHog providers and the dashboard web SDK). This document
covers the **research experiment** side: exposing typed autoresearch
experiment configuration through the same CNCF standard so external tooling
can consume it. The two layers share the `openfeature-sdk` core dependency
and nothing else — neither replaces the other, and neither touches ship
gates.
...
1. **flagd export** — `export_flagd_flags(experiments, flag_set_id=...)`
   ...
   variant and returns `null` (→ code default) otherwise. The artifact is
   consumable by flagd and every OpenFeature SDK.
2. **In-process provider** — `ExperimentFlagProvider` implements the
   ...
   (`client.get_integer_details("steps", 200, ctx)`). Uses the
   `openfeature-sdk` core dependency (core since the product experiment
   layer landed in #692/#703).
...
- **Selection stays explicit.** OpenFeature here *represents* experiment
  ...
  mechanisms. (Runtime traffic-split rollout is the product layer's job —
  see `openfeature-experiments.md`.)
