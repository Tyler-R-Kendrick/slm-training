...
- `tests/test_dsl/test_surface_realization.py` — neural realizer plugged into
...
- `tests/test_data/test_progspec.py` — `derive_surface_realization_records`
  emits one record per surface slot, respects `include_authorities`, inherits
  split/group, and can use a trained neural realizer.
