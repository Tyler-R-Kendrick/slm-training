...
| --- | --- | --- | --- |
| 2026-07-20 | `e621-cli-gap-treatment` | No (`--no-sync-checkpoints`) | 20-step CPU scratch **TwoTower** run (`--context-backend scratch --output-tokenizer choice`) against a fresh fixture corpus built this session (`outputs/data/train/e621-cli-gap-scratch`), 91.18s under `max_wall_minutes=3`, SHA `9ed5efbf6b9977dfc23e4b70fc8a5f79581c31cb55b2be58452f7af7c0f6a500`. Verifies E621's `scripts/train_model.py` CLI-wiring fix: `--schema-role-slot-decode-weight 8 --semantic-plan-decode-weight 4 --semantic-plan-repeated-array-close-margin-decode-weight 2` now persist as nonzero in the live run's own `scoreboard.json` `evaluation_policy` and decode traces (previously always `0.0`/`None` — no CLI path existed). Matched against `e621-cli-gap-control-choice` below (`n=3` smoke): meaningful v1/fidelity/reward improve, structural similarity regresses; not a ship claim. JSON: [iter-e621-train-model-decode-weight-cli-gap-20260720.json](iter-e621-train-model-decode-weight-cli-gap-20260720.json). |
| 2026-07-20 | `e621-cli-gap-control-choice` | No (`--no-sync-checkpoints`) | Matched control for the row above: identical 20-step CPU scratch recipe and fixture corpus, all E621-added decode weights left at default. 91.71s under `max_wall_minutes=3`, SHA `026a9df86150173c15db1ab8d584114955e913bb486eb5a07c85ac7027539223`. Confirms the pre-fix baseline (`evaluation_policy.schema_role_slot_decode_weight`/`semantic_plan_decode_weight` = `0.0`). Not promotable or ship. |
