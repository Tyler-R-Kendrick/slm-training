...
| 2026-07-20 | `e619-slot-contract-in-context-scratch80-20260720` (documented as **E620**, see note) | No (`--no-sync-checkpoints`) | Fresh CPU scratch **TwoTower** loop from the current checkout, replaying E616/E617/E619's exact recipe (this sandbox's `outputs/` started empty again); ran on published E530 (244 records) in 10.26s under `max_wall_minutes=3`; loss 26.5243, 1,643,522 trainable params, checkpoint SHA `119dd41a57573fe408ee1208a9159cafca9a45c7050bd00274571938898a8539` (byte-identical to E616/E617/E619, confirming a true replay), full-state SHA `36293888872aa397c210ef715245c8c33378a1271b8fb2ea3665c3570bb527f0`. This experiment is documented as **E620** (not E619) because a separate, concurrent session independently landed its own `## E619` section on this same branch first; E620's live control (flag genuinely unset, unlike E619's concurrent session which had the flag on in both its arms) shows `binding_aware_meaningful_v2_coverage` rising 0.75→1.0 as `ood_gallery_01`'s `required_inventory_coverage` moves from `UNKNOWN` to a real, correct `FAIL` — correcting E618's and E619's "every/all 4 records" framing to "`ood_gallery_01` only". Every headline quality metric is unchanged and strict v2 stays 0.0 in both arms. JSON: [iter-e620-slot-contract-in-context-control-20260720.json](iter-e620-slot-contract-in-context-control-20260720.json). Not promotable or ship. |
