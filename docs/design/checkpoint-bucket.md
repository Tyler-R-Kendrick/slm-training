...
| --- | --- | --- | --- |
| 2026-07-24 | `slm230_bounded_recursive_r4_r2` | No (`--no-sync-checkpoints`) | Local four-step CPU scratch shared-recursive diagnostic on 97 strict symbol-only records; 1.70s under `max_wall_minutes=3`, SHA `1604b2cb9282928fa0969ecbbe7d78c9aa4b9907f74d0d58936bfc298a88b28a`. Held-out free-running quality is zero at every depth and the recurrence verdict is `stagnant`, so upload and promotion are explicitly unwarranted. |
| 2026-07-22 | `e727-symbol-only-binder-arity140-r1` | No (`--no-sync-checkpoints`) | Local output-contract-v2 CPU scratch binder-arity diagnostic; 140 steps in 77.46s under `max_wall_minutes=2`, SHA `c211d2eae1028334a33d16adc2c29b26a908ade3f90e8c86c2d3da914136a857`. Weights 1/2 change no smoke or held-out choices and strict meaning remains 0.0, so no upload or promotion. |
| 2026-07-22 | `e729-symbol-only-binder-topology140-r1` | No (`--no-sync-checkpoints`) | Local output-contract-v2 CPU scratch binder-topology diagnostic; 140 steps in 77.50s under `max_wall_minutes=2`, SHA `c5bafb8d88a0897e3c9c2d4727b04134042ae2944cdeabcb3c65fb7a9d18c43d`. Weights 0.25/1 regress smoke quality and strict meaning remains 0.0, so no upload or promotion. |
| 2026-07-22 | `e726-symbol-only-root-arity140-r1` | No (`--no-sync-checkpoints`) | Invalid local output-contract-v2 CPU scratch attempt; 140 steps in 81.20s under `max_wall_minutes=2`, SHA `d84148fe327c18dee6a4ad4957b1b23499e17ae364c79a97cdc8150503a1b91b`. The choice-only arity treatment was not instantiated on lexer and all tensors match E723, so the checkpoint must never be evaluated, uploaded, promoted, or served. |
| 2026-07-22 | `e725-symbol-only-component-inventory130-r1` | No (`--no-sync-checkpoints`) | Local output-contract-v2 CPU scratch cumulative inventory diagnostic; 130 steps in 73.22s under `max_wall_minutes=2`, SHA `897208bf4bf0ce12b137145a3a6c88f2140faa6579080b0fe54c6794fde8ba1e`. Inventory weight 1/0 is prediction-identical and smoke meaning-v1/strict-v2 is 0.0, so no upload or promotion. |
| 2026-07-21 | `e714-symbol-only-scratch600-r1` | No (`--no-sync-checkpoints`) | First output-contract-v2-compatible local CPU scratch baseline; 600 steps in 48.72s under `max_wall_minutes=2`, SHA `71ef1d25efb5186b885e8ee9e1370002f419bf42ea53985111734c49cfd2b49e`. Five-suite strict meaning is 0.0 and AgentV 0/5, so no upload or promotion. |
| 2026-07-21 | `e720-symbol-only-component-inventory600-r1` | No (`--no-sync-checkpoints`) | Local output-contract-v2 CPU scratch inventory diagnostic; 600 steps in 72.38s under `max_wall_minutes=2`, SHA `842a1a21fb9897fe5ee594d9c9d2835315d63d4a12905e3c3640eec348f91a11`. Smoke strict meaning is 0.0 in both decode arms and weight 4 times out 3/3, so no upload or promotion. |
| 2026-07-21 | `e721-symbol-only-component-plan190-r4` | No (`--no-sync-checkpoints`) | Local output-contract-v2 CPU scratch role/count-plan diagnostic; 190 steps in 90.39s under `max_wall_minutes=2`, SHA `c30fd565fced08626f39af5e9e23d233d88c26e0dac3b031928105b97c20f530`. Smoke parse is 1.0 but strict meaning is 0.0 and plan on/off is identical, so no upload or promotion. Three larger timed-out attempts are invalid and produced no accepted checkpoint evidence. |
| 2026-07-21 | `e722-symbol-only-component-edge150-r1` | No (`--no-sync-checkpoints`) | Local output-contract-v2 CPU scratch plan+edge diagnostic; 150 steps in 77.52s under `max_wall_minutes=2`, SHA `08873bf0940eec19d0e90f50bfbd801f8547b45e450fa6379abe21c90a25597d`. Smoke strict meaning is 0.0 and edge on/off outputs are identical, so no upload or promotion. |
| 2026-07-21 | `e723-symbol-only-slot-owner140-r1` | No (`--no-sync-checkpoints`) | Local output-contract-v2 CPU scratch slot-owner diagnostic; 140 steps in 77.16s under `max_wall_minutes=2`, SHA `787d2d21d7c29d56637355fd364f16a0d67b1f452fc0f4ce3a7d486b2bd62795`. Slot decode causally improves smoke and held-out, but strict meaning remains 0.0, so the lever is retained without uploading or promoting the checkpoint. |
