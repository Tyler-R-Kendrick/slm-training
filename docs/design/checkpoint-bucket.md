...
| 2026-07-20 | `e615_training_loop_twotower_scratch_20260720` | No (`--no-sync-checkpoints`) | Fresh CPU scratch **TwoTower** loop from the current checkout (E615 object-frame `schema_role_slot_decode_weight` change); ran 8 steps on published E530 (244 records) in 2.454s under `max_wall_minutes=3`; loss 37.9045, 1,643,522 trainable params, checkpoint SHA `c083f8d1710e3d6cb20890d6e1805692847f3ebc20aed1db0be9e573eef0c508`, full-state SHA `2c924586e6faee79b3d884f5b5721f982d847f2150df35ea718d1eb583a60407`. Unlike the E540-E548 wiring runs, a matched OOD `n=4` control (`schema_role_slot_decode_weight=0`) vs treatment (`=8.0`) eval pair *was* run against this checkpoint (`e615-control-eval-r1` / `e615-treatment-eval-r1`); both are byte-identical with 0/4 parseable predictions because the checkpoint is too undertrained to complete constrained decode, so the Gallery `src`/`alt`/`details` fix could not be observed end-to-end. JSON: [iter-e615-object-frame-schema-role-slot-20260720.json](iter-e615-object-frame-schema-role-slot-20260720.json). Not promotable or ship. |
...
| 2026-07-20 | `e619-slot-contract-in-context-scratch80-20260720` (documented as **E621**, see note) | No (`--no-sync-checkpoints`) | Fresh CPU scratch **TwoTower** loop from the current checkout, replaying E616/E617/E619's exact recipe (this sandbox's `outputs/` started empty again); ran on published E530 (244 records) in 10.26s under `max_wall_minutes=3`; loss 26.5243, 1,643,522 trainable params, checkpoint SHA `119dd41a57573fe408ee1208a9159cafca9a45c7050bd00274571938898a8539` (byte-identical to E616/E617/E619, confirming a true replay), full-state SHA `36293888872aa397c210ef715245c8c33378a1271b8fb2ea3665c3570bb527f0`. This experiment is documented as **E621** (not E619) because two separate, concurrent sessions independently landed their own `## E619` and `## E620` sections on this same branch while it was in progress (the `## E620` one is a different topic, a `semantic_plan_*` decode-weight gating gap). E621's live control (flag genuinely unset, unlike E619's concurrent session which had the flag on in both its arms) shows `binding_aware_meaningful_v2_coverage` rising 0.75→1.0 as `ood_gallery_01`'s `required_inventory_coverage` moves from `UNKNOWN` to a real, correct `FAIL` — correcting E618's and E619's "every/all 4 records" framing to "`ood_gallery_01` only". Every headline quality metric is unchanged and strict v2 stays 0.0 in both arms. JSON: [iter-e621-slot-contract-in-context-control-20260720.json](iter-e621-slot-contract-in-context-control-20260720.json). Not promotable or ship. |
