...
| [egg / e-graphs](https://arxiv.org/abs/2004.03082) (Willsey et al., POPL 2021) | **Adjacent** | Post-realization canonicalization is motivated by equality saturation, but the contract is not an e-graph / equality-saturation engine (cf. `iter-e252-canonicalizer-20260716.md`). **SLM-241 (GRT0-01)** scaled the canonicalizer's own documented normal-form claim (idempotent, always-valid, alpha-invariant under non-root binder renaming) from the 3 hand-picked `tests/test_dsl/test_canonicalize.py` examples to a 150-candidate coverage-guided generator corpus across 3 seeds: **`ceiling_confirmed_at_scale`**, 0 counterexamples (`iter-slm241-grt0-01-grammar-round-trip-alpha-invariance-20260721.md`). Only the in-process Lark backend was exercised (the official lang-core Node bridge was unavailable in the run sandbox), so cross-backend parity with the official parser remains untested. |
