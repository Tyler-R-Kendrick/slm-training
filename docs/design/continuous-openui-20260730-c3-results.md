...
2. At 40 steps, `meaningful_program_rate` rose to 0.333 for both arms (from
   0.0 at 20 steps in cycle 2), but the control/candidate delta stayed null —
   `grammar_completion_bounds` + `compact_active_canvas` together still did
   not improve smoke p50 latency or mpr at the larger step budget.
...
1. **model:** do not promote bounds/canvas on latency alone; mpr stayed flat
   between control and candidate at both 20 and 40 steps.
