...
| **E62** | Minimal hard negatives | **Wired:** the deterministic `data/corrupt` taxonomy emits verified clean-target repair rows across lexical, grammar, schema, reference-graph, dataflow, and patch failures | Use [`data/corrupt`](../../src/slm_training/data/corrupt/) cases for repair / preference / RL inputs; ambiguous repairs carry multiple accepted targets and are excluded from exact-repair claims |
