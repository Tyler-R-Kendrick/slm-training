...
- **`plan_oracle_substitutor`**: Distinct from semantic_repair's corruption-repair targets: this substitutes ground-truth oracle values for factor-wise diagnosis, not corruption recovery. VCE-004 added an immutable paired baseline/intervention observation record (`PlanInterventionRecordV1`) and manifest-safety filtering on top of the same substitutor. VCE-005 extended `PlanSource` with `destructive`/`random` (permutes baseline's own existing per-factor content -- matched serialization/work, never a second plan's content, never contamination-bannered), added `select_shuffled_oracle` (content-only compatibility, never `identity.source_program_fingerprint`/`prompt_context_hash`), and `build_baseline_intervention` (an explicit, first-class matched-telemetry no-intervention control). No new record schema.
...
- **`symbolic_expr_corpus`**: SRP-008/SLM-471. The row predicted `new_owner_justified: false` ("Reuses `data_corpus_audit` leakage-control machinery for a new corpus, not a new owner"); the generation contract itself isn't literally defined in `scripts/audit_data_corpora.py`, so it is its own subsystem instead (documented divergence, same precedent as SGS-008/SRP-004/SRP-005/SRP-007) — but the row's actual constraint is honored literally: `scripts/audit_data_corpora.py` gained `symbolic_regression_corpus_leakage_report()`/`--mode symbolic-regression-audit` (bumping `data.corpus_certification` v3→v4), calling this module's `generate_corpus`/`audit_corpus_leakage` rather than a second leakage checker. Canonical-family split assignment reuses `harnesses/train_data/split_policy.py`'s `RootFamilySplitPolicyV1`, keyed on the canonicalized expression's hash, so equivalent expressions are assigned a split before any numeric row is sampled. Coefficient placement is guaranteed affine by construction; a candidate whose true coefficients are not uniquely identifiable from its own train rows (collinear basis terms) is filtered as `collinear_coefficient_terms` rather than kept with non-recoverable ground truth. The true equation/coefficients live only in `CorpusOracleV1` — the generated `SymbolicRegressionProblemV1`'s `provenance` is always empty.
- **`symbolic_expr_enumerate`**: SRP-009/SLM-472. The row predicted `extension_points: [valid_state_search_generic]` (the fixed-hole finite-domain CSP controller). That controller's state model is a fixed set of holes each drawn from a finite domain, solved via backtracking + certificate-backed closure — it does not naturally represent "generate a variable-shaped tree up to a depth/node budget" without an artificial fixed-arity template encoding, whereas `symbolic_expr_ir`'s own arity table/budgets already are the natural generative model; registered as its own subsystem instead (documented divergence, same precedent as SGS-008/SRP-004/SRP-005/SRP-007). Composes rather than re-derives: generation reads only `symbolic_expr_ir`'s operator arity table (never targets/models), canonical identity/dedup comes from `symbolic_expr_canonicalize.canonicalize_expr`, and every unique candidate is scored through `symbolic_expr_report.build_evidence_record`/`pareto_front` unmodified. Deduplicates by canonical hash before evaluation. A truncated enumeration (`max_states` cap reached before natural exhaustion) is structurally forbidden from claiming `optimality_claim="exhaustive_under_declared_bounds"` — `EnumerativeBaselineResultV1.__post_init__` raises on the mismatched combination, not just documented convention.
...
| **SRP-010** — Add end-to-end symbolic-pack portability, default-isolation, and standards fixtures | SLM-474 | `dsl_pack_registry_canonical` | no | Extends the canonical pack registry's portability guarantees to the SRP-001 pack. Fixture evidence: [iter-slm474-srp-010-portability-20260810.md](iter-slm474-srp-010-portability-20260810.md) (`srp010_fixture`; OpenUI remains default). Campaign successor: RSP-008 / EXP-SR-12 (SLM-487). |
...
| **RSP-008** — Run real OpenUI + symbolic-regression second-pack portability certification (EXP-SR-12) | SLM-487 | `dsl_pack_registry_canonical`, `experiment_campaign` | no | Campaign execution successor to SRP-010/SLM-474: runs the portability work through ExperimentCampaignV1 (not the fixture), following the SLM-159 replication precedent. Fixture precursor: [iter-slm474-srp-010-portability-20260810.md](iter-slm474-srp-010-portability-20260810.md). |
