# autotrain_wf_smoke_20260727_steps32_seed0

**Honesty:** fixture_or_scratch. **Not ship.**

train_version=wf_smoke_v2 steps=32 seed=0 last_loss=8.38817024230957 stopped_on=steps wall=8.79 record_count=101
...
python -m scripts.train_model --train-dir src/slm_training/resources/data/train/wf_smoke_v2 \
  --model twotower --context-backend scratch --steps 32 \
  --run-id autotrain_wf_smoke_20260727_steps32_seed0 --no-sync-checkpoints --device cpu --seed 0
```
...
`outputs/runs/autotrain_wf_smoke_20260727_steps32_seed0/train_summary.json` (not committed; `outputs/` is gitignored).
