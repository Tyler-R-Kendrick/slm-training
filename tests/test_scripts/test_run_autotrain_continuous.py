...
def test_supervisor_noops_when_regime_parked(
    ...
    rc = supervisor.main(
        [
            ...
            "--max-cycles",
            "3",
            # Park is a wait state (default 300 s backoff per cycle): keep the
            # three parked re-checks to seconds so the changed-test hook fits
            # its wall.
...
def test_fit_screening_decode_carries_certified_sample_size_report() -> None:
    ...
    if report["verdict"] == "feasible":
        ...
        # policy.v3 (P2): an unmeasured same-metric paired SD is advisory,
        # never a borrowed power floor -- the screen spends the affordable n.
        assert report["power_floor_status"] == "unmeasured"
        assert report["must_generate"] is False
        assert int(report["chosen_n"]) == int(meta["smoke_n"]) >= 6
    else:
...
# --- P8: measured cold-start prior, baseline_seed champion, two-cycle warm start
