"""Tests for the SLM-235 annotation export pairing-mechanism consistency
probe CLI."""
...
from scripts.run_slm235_annotation_export_pairing_consistency import main


def test_plan_only_mode(tmp_path: Path) -> None:
    rc = main(["--mode", "plan-only", "--output-dir", str(tmp_path)])
    assert rc == 0
    report = tmp_path / "slm235_annotation_export_pairing_consistency_report.json"
    assert report.is_file()
    payload = json.loads(report.read_text(encoding="utf-8"))
    assert payload["status"] == "plan_only"
    assert payload["matrix_set"] == "slm235_annotation_export_pairing_consistency"
