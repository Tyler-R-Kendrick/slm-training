"""Tests for the SLM-228 RL-readiness reward-variance gate stress test CLI."""
...
from scripts.run_slm228_rl_readiness_variance_gate import main


def test_plan_only_mode(tmp_path: Path) -> None:
    rc = main(["--mode", "plan-only", "--output-dir", str(tmp_path)])
    assert rc == 0
    report = tmp_path / "slm228_rl_readiness_variance_gate_report.json"
    assert report.is_file()
    payload = json.loads(report.read_text(encoding="utf-8"))
    assert payload["status"] == "plan_only"
    assert payload["matrix_set"] == "slm228_rl_readiness_variance_gate"
