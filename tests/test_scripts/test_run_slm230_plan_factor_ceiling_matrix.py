"""Tests for the SLM-230 plan-factor oracle-substitution ceiling matrix CLI."""
...
from scripts.run_slm230_plan_factor_ceiling_matrix import main


def test_plan_only_mode(tmp_path: Path) -> None:
    rc = main(["--mode", "plan-only", "--output-dir", str(tmp_path)])
    assert rc == 0
    report = tmp_path / "slm230_plan_factor_ceiling_matrix_report.json"
    assert report.is_file()
    payload = json.loads(report.read_text(encoding="utf-8"))
    assert payload["status"] == "plan_only"
    assert payload["matrix_set"] == "slm230_plan_factor_ceiling_matrix"
    assert payload["rows"] == []
...
def test_fixture_mode_writes_design_docs(tmp_path: Path) -> None:
    ...
    assert payload["disposition"] == "ceiling_confirmed_joint_requirement"
    md_text = design_md.read_text(encoding="utf-8")
    assert "SLM-230" in md_text
