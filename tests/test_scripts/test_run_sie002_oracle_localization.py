"""CLI smoke for scripts.run_sie002_oracle_localization."""
...
from scripts.run_sie002_oracle_localization import main
...
def test_plan_only_exits_zero(capsys) -> None:
    ...
    assert payload["manifest"]["experiment_id"] == "exp-sr-1"
    assert payload["promotion"] is False


def test_fixture_writes_docs(tmp_path: Path, capsys) -> None:
    docs_out = tmp_path / "iter-slm476-sie-002.json"
    ...
    assert payload["kind"] == "sie002_oracle_localization_fixture/v1"
    ...
    assert "authorized_factors=" in captured
