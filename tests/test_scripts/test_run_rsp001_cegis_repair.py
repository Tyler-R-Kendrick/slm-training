"""CLI smoke for scripts.run_rsp001_cegis_repair."""
...
from scripts.run_rsp001_cegis_repair import main
...
def test_plan_only_exits_zero(capsys) -> None:
    ...
    assert payload["manifest"]["experiment_id"] == "exp-sr-4"
...
def test_fixture_writes_docs(tmp_path: Path, capsys) -> None:
    docs_out = tmp_path / "iter-slm488-rsp-001.json"
    ...
    assert payload["kind"] == "rsp001_cegis_repair_fixture/v1"
    ...
    assert payload["invariants"]["default_off"] is True
    assert docs_out.with_suffix(".md").is_file()
    captured = capsys.readouterr().out
    assert "verified_repair_yield=" in captured
