"""Tests for the SLM-238 gradient-accumulation equivalence sweep CLI."""
...
from scripts.run_slm238_gradaccum_equivalence import main
...
def test_plan_only_mode(tmp_path: Path) -> None:
    ...
    assert rc == 0
    report = tmp_path / "slm238_gradaccum_equivalence_report.json"
    assert report.is_file()
    payload = json.loads(report.read_text(encoding="utf-8"))
    assert payload["status"] == "plan_only"
    assert payload["matrix_set"] == "slm238_gradaccum_equivalence"
