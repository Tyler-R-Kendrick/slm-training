...
def test_stop_real_owned_session_leaves_unrelated_session_alive(tmp_path):
    ...
    try:
        ...
        assert process_identity(worker_pid) == ""
...
def test_start_and_stop_use_real_process_with_disposable_controller_fixture(tmp_path, monkeypatch):
    ...
    # The release-sandbox tmpfs is smaller than the production 256 MiB free
    # floor; that floor contract has dedicated probes above, while this case
    # verifies launch, identity, locks and stop. Keep the writability and
    # memory legs of the real probe intact.
    monkeypatch.setattr(
        control,
        "storage_health",
        lambda root: readiness.storage_health(root, minimum_free_bytes=0),
    )
    source = tmp_path / "source"
