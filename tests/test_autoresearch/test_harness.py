...
def test_ack_action_deliver_stack_accepts_bound_skip_evidence(tmp_path: Path) -> None:
    ...
    # Evidence is resolved relative to the acting campaign's own directory
    # (see `_file_evidence` in storage.py), so the URI is bare here.
    ...
    with pytest.raises(ValueError, match="evidence must be a Git commit"):
    ...
    # Evidence paths are scoped to the acting campaign's own directory, so
    # the only way to even attempt substitution is for that campaign to
    # carry its own copy of the file (e.g. one that was mistakenly copied
    # over) -- which must still be rejected because its `campaign_id` does
    # not match the campaign acknowledging the action.
    ...
    (other_dir / "sdlc_delivery.json").write_text(
        (camp_dir / "sdlc_delivery.json").read_text(encoding="utf-8")
    )
