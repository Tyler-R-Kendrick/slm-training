...
def hypothesis_matrix(count: int = 5) -> HypothesisMatrix:
    citations = (
        ...
        candidates.append(
            HypothesisCandidate(
                experiment=experiment(
                    experiment_id=f"hyp-{index}",
                    hypothesis=f"Distinct grounded hypothesis number {index} improves structure.",
                    citations=citations,
                    knobs=ExperimentKnobs(steps=100 + index),
                ),
                evidence_uses=(
                    EvidenceUse(
                        role="research",
                ...
                novelty=CategoricalNoveltyAudit(
                    ...
                    residual_elements=(f"candidate mechanism {index}",),
...
def test_campaign_rejects_hypothesis_floor_above_budget() -> None:
    with pytest.raises(ValidationError, match="min_hypotheses"):
        CampaignSpec(
            campaign_id="too-small",
            objective="Test an invalid hypothesis budget.",
