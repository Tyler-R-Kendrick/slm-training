...
from slm_training.dsl.surface import SurfaceAuthority, SurfaceSlotKind
...
def _tiny_surface_ar_config() -> SurfaceAutoregressorConfig:
    return SurfaceAutoregressorConfig(d_model=32, n_layers=1, n_heads=2, max_len=64)


def _trained_neural_realizer_for_spec(spec: ProgramSpec) -> NeuralSurfaceRealizer:
    """Train a tiny fixture model to emit a custom name for the spec's binder."""
    pack = get_pack("openui")
    slots = pack.require("surface_slot_extractor")(spec.canonical_openui)
    slot = next(
        s
        for s in slots
    ...
    model = SurfaceAutoregressor(_tiny_surface_ar_config())
    train_surface_autoregressor(
        model, [(prompt, "mytitle")], steps=300, lr=5e-3, seed=0
    )
    model.eval()
