...
def test_slot_coverage_close_bias_descends_before_wrong_property_slot() -> None:
    from types import SimpleNamespace

    model = _model(output_tokenizer="choice", slot_coverage_close_decode_weight=4.0)
    tokenizer = model.tokenizer
    slice_id = tokenizer.token_to_id["+Slice"]
    slot_id = tokenizer.sym_id(0)
    close_id = tokenizer.token_to_id["]"]
    state = SimpleNamespace(
        frames=[
            SimpleNamespace(
                kind="component", expr_type="element:Carousel", arg_index=0
            ),
            SimpleNamespace(
                kind="variadic",
                expr_type="array",
                schemas=({"anyOf": [{"$ref": "#/$defs/Slice"}]},),
                close="]",
            ),
        ]
    )
    ...
    bias = model._slot_coverage_close_bias(
        state,
        [tokenizer.bos_id],
        (slice_id, slot_id, close_id),
        torch.tensor([1.0, 5.0, 4.0]),
        [":dashboard.metric.value"],
        {":dashboard.metric.value": ("Slice",)},
    )
