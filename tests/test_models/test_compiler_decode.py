...
def test_repeated_plan_slot_bias_retargets_precontent_schema_floor() -> None:
    """E634: compose the schema-opaque pre-content floor with the
    repeated-instance margin by retargeting it at the legal empty literal,
    instead of letting it independently float a visible slot there. This is
    default-off evidence: on the checkpoint available for E634's replay this
    correction collapsed the whole repeated family's decode (see
    docs/design/iter-e634-composed-role-scoring-20260721.md), but the unit
    behavior below is what the composed bias computes in isolation."""
    from types import SimpleNamespace

    from slm_training.dsl.production_codec import LIT_PREFIX

    name_schema = {"type": "string"}
    placeholder_schema = {"type": "string", "x-openui-placeholder": True}

    def _make_state(arg_index: int) -> SimpleNamespace:
        return SimpleNamespace(
            frames=[
                SimpleNamespace(
                    kind="component",
                    expr_type="element:Input",
                    schemas=(name_schema, placeholder_schema),
                    arg_index=arg_index,
                    required_args=2,
                )
            ]
    ...
    # Composed (schema_opaque_decode_weight > 0): the first repeated
    # instance's pre-content `.name` argument floors the legal empty literal
    # instead of the margin placing a visible slot there.
    composed = _model(
        output_tokenizer="choice",
        semantic_plan_repeated_slot_margin_decode_weight=2.0,
        schema_opaque_decode_weight=4.0,
    )
    input_id = composed.tokenizer.token_to_id["+Input"]
    literal_id = composed.tokenizer.token_to_id[f'{LIT_PREFIX}""']
    composed._semantic_plan_action_counts = [{input_id: 2}]
    composed._slot_contracts = [[":auth.name", ":auth.email"]]
    slot0 = composed.tokenizer.sym_id(0)
    slot1 = composed.tokenizer.sym_id(1)
    prefix = [composed.tokenizer.bos_id, input_id]
    candidates = (slot0, slot1, literal_id)
    name_arg_state = _make_state(arg_index=0)

    bias = composed._semantic_plan_repeated_slot_bias(
        0, name_arg_state, prefix, candidates, torch.tensor([9.0, 10.0, 1.0])
    )

    assert bias is not None
    assert bias.tolist() == [0.0, 0.0, 11.0]

    # Without the legal literal available as a candidate there is nothing
    # correct to retarget onto, so the margin abstains entirely rather than
    # falling back to the old slot-flooring behavior.
    assert (
        composed._semantic_plan_repeated_slot_bias(
            0, name_arg_state, prefix, (slot0, slot1), torch.tensor([9.0, 10.0])
        )
        is None
    )

    # The retargeting is specific to the pre-content argument: the content
    # property itself (arg_index=1) is unaffected and still floors the best
    # unused visible slot as before.
    content_arg_state = _make_state(arg_index=1)
    bias = composed._semantic_plan_repeated_slot_bias(
        0, content_arg_state, prefix, candidates, torch.tensor([9.0, 10.0, 1.0])
    )
    assert bias is not None
    assert bias.tolist() == [0.0, 2.0, 0.0]

    # Without the schema-opaque treatment enabled, the pre-E634 behavior is
    # unchanged: the margin still independently floors a slot at the
    # pre-content argument (uncomposed, broader lever untouched).
    uncomposed = _model(
        output_tokenizer="choice",
        semantic_plan_repeated_slot_margin_decode_weight=2.0,
    )
    uncomposed_input_id = uncomposed.tokenizer.token_to_id["+Input"]
    uncomposed._semantic_plan_action_counts = [{uncomposed_input_id: 2}]
    uncomposed._slot_contracts = [[":auth.name", ":auth.email"]]
    uncomposed_prefix = [uncomposed.tokenizer.bos_id, uncomposed_input_id]
    uncomposed_slot0 = uncomposed.tokenizer.sym_id(0)
    uncomposed_slot1 = uncomposed.tokenizer.sym_id(1)
    bias = uncomposed._semantic_plan_repeated_slot_bias(
        0,
        name_arg_state,
        uncomposed_prefix,
        (uncomposed_slot0, uncomposed_slot1),
