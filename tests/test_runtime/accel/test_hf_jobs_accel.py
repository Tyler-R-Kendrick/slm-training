...
from slm_training.levers import HF_JOB_TIMEOUT, INTERRUPT_AFTER_SECONDS
...
def test_hf_jobs_train_dry_run() -> None:
    ...
    assert plan["timeout"] == HF_JOB_TIMEOUT
...
def test_hf_jobs_rejects_overlong_timeout() -> None:
    with pytest.raises(ValueError, match=f"must be {HF_JOB_TIMEOUT}"):
        hf_jobs_train.build_hf_jobs_command(
            flavor="a100-large",
            timeout=f"{int(HF_JOB_TIMEOUT[:-1]) + 1}m",
            image="pytorch/pytorch:2.5.1-cuda12.4-cudnn9-runtime",
            entrypoint="echo hi",
            checkpoint_bucket="hf://buckets/TKendrick/OpenUI",
            mount_bucket=False,
...
def test_remote_train_includes_fast_train() -> None:
    ...
    assert f"timeout --signal=INT --kill-after=10s {INTERRUPT_AFTER_SECONDS}s" in script
